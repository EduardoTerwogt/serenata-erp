...
export function QuotationTotalsPanels({
  ...
  storedPorcentajeFee,
  storedIvaActivo,
}: QuotationTotalsPanelsProps) {
  const feePercent = ((storedPorcentajeFee ?? porcentajeFee) * 100).toFixed(0)
  const ivaVisible = editable ? ivaActivo : (storedIvaActivo ?? ivaActivo)
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between text-sm gap-3">
            ...
            <span className="text-white text-right">${fmt(totals.subtotal)}</span>
          </div>
          ...
          <div className="flex justify-between text-sm items-start gap-3">
              ...
              {editable ? (
                <>
                  <input
                    ...
                    max="100"
                    step="0.5"
                    value={(porcentajeFee * 100).toFixed(1)}
            ...
            <span className="text-white text-right flex-shrink-0">${fmt(totals.fee_agencia)}</span>
          </div>

          <div className="flex justify-between text-sm gap-3">
            <span className="text-gray-400">General</span>
            <span className="text-white text-right">${fmt(totals.general)}</span>
          </div>
            ...
            <span className="text-gray-400 flex items-center gap-2 flex-wrap flex-1 min-w-0">
              ...
              {editable && (
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${ivaActivo ? 'bg-blue-600' : 'bg-gray-600'}`}
                >
                  <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${ivaActivo ? 'translate-x-4' : 'translate-x-1'}`} />
                </button>
              )}
            </span>
            <span className={`${ivaVisible ? 'text-white' : 'text-gray-600'} text-right flex-shrink-0`}>
              ${fmt(totals.iva)}
            </span>
          </div>
          ...
          {editable ? (
            <div className="flex justify-between text-sm items-start gap-3">
              ...
              <span className={`${totals.descuento > 0 ? 'text-yellow-400' : 'text-gray-600'} text-right flex-shrink-0 pt-0.5`}>
                {totals.descuento > 0 ? `-$${fmt(totals.descuento)}` : '$0.00'}
              </span>
            </div>
          ) : totals.descuento > 0 ? (
            <div className="flex justify-between text-sm gap-3">
              <span className="text-gray-400">Descuento</span>
              <span className="text-yellow-400 text-right">-${fmt(totals.descuento)}</span>
            </div>
          ) : null}

          <div className="border-t border-gray-700 pt-2 mt-1 flex justify-between font-bold gap-3">
            <span className="text-white">TOTAL</span>
            <span className="text-green-400 text-lg text-right">${fmt(totals.total)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 md:p-6">
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm gap-3">
            ...
            <span className={`${totals.margen_total >= 0 ? 'text-green-400' : 'text-red-400'} text-right`}>
              ${fmt(totals.margen_total)}
            </span>
          </div>
          <div className="flex justify-between text-sm gap-3">
            <span className="text-gray-400">Fee Agencia</span>
            <span className="text-white text-right">${fmt(totals.fee_agencia)}</span>
          </div>
          <div className="border-t border-gray-700 pt-2 mt-1 flex justify-between font-semibold gap-3">
            ...
            <span className={`${totals.utilidad_total >= 0 ? 'text-green-400' : 'text-red-400'} text-right`}>
              ${fmt(totals.utilidad_total)}
            </span>
          </div>
          {totals.subtotal > 0 && (
            <div className="flex justify-between text-sm gap-3">
