...
| F14c | **Activado** — `lpad(n::text, 3, '0')` trunca (no rellena) folios de 4+ dígitos: con volumen≥1000 `reserve_next_cotizacion_folio`/`preview_next_cotizacion_folio_principal` devuelven 'SH100' en vez de 'SH1000', chocando con el folio real #100. Encontrado real al sembrar 995 cotizaciones secuenciales en `serenata-erp-test` (Bloque 4/3C-4) | 3B-7 |
| F14d | **Activado** — `reserve_next_cotizacion_folio`/`preview_next_cotizacion_folio_principal` calculan "ocupado" solo con reservas ACTIVAS (`consumed_at IS NULL`) + `cotizaciones.id` vivas; una reserva ya consumida cuya cotización se borra después (`DELETE /api/cotizaciones/[id]`, real en producción para BORRADOR) deja de contar en ambas ramas, pero `cotizacion_folio_reservations.folio` es UNIQUE permanente -- el folio queda bloqueado para siempre. Encontrado real en `serenata-erp-test` (`SH2201`, generado y borrado por el propio test suite `live`, que ya documentaba este hueco en `tests/e2e/utils/live-cleanup.ts` sin saber que también aplica a producción vía el DELETE real) | 3B-7 |
| F15 | `rate_limits` sin retención | 3C-4 |
| F15b | *(condicional)* `syncAllDown()` no cabe en `maxDuration` del cron de keep-alive al volumen objetivo — **no evaluado**: decisión explícita del usuario (2026-09-16) saca la validación de duración de Sheets del alcance de Engineering Hardening (decisión de producto a tomar aparte); nunca llegó a medirse con un `spreadsheetId` válido | 3C-4 (fuera de alcance) |
| F15c | **Activado, corregido el gate, validación empírica fuera de alcance** — gate de 3C-4 (`measure-sync-down-duration.mjs`) pasaba con `duración=11.22s` (bajo el umbral) pero `totalRows=0 errors=9 state=error`: `syncAllDown()` fallaba en las 9 tablas contra el entorno serverless real con "Requested entity was not found" (Google Sheets API) — el gate solo miraba duración, nunca `errors`/`state`. Causa raíz: `TEST_SHEETS_SPREADSHEET_ID` no es accesible con las credenciales de Google del entorno de loadtest — nadie lo había ejercitado antes (`live` de `e2e.yml` nunca usa Sheets). Corregido el gate en sí (ahora falla también si `errors>0`) y agregado `reset-sheets-integration.mjs`/input `reset_sheets_integration`, que confirmó la integración funcional con un spreadsheet nuevo (`totalRows=37572 errors=0`) -- pero decisión explícita del usuario saca de EF-3 la validación de duración/producto de Sheets, así que ese spreadsheet nuevo no se activó formalmente | 3C-4 (fuera de alcance) |
...
de cada fila 3D-0b..3D-12 apunta a esa rama/PR compartida; "Estado" se queda
en `Pendiente` (el progreso real -- código+tests listos localmente para
cuáles bloques -- vive en "Nota") hasta que ese PR compartido mergea a
`main`, momento en el que TODAS las filas bundleadas pasan a `Cerrado` de
una vez, con el mismo "Commit de merge". Nunca `En curso` mientras dependa de
otro bloque de este mismo lote sin mergear -- `scripts/validate-ef3-tracker.mjs`
...
(3D-0b, 3D-1, 3D-2, 3D-3, 3D-4, 3D-6, 3D-7, 3D-9, 3D-10, 3D-11, 3D-12 --
11 filas en total). **3D-5 quedó fuera del bundle**, sigue `Pendiente`
(pausado por decisión explícita del usuario, ver su propia fila) -- no
bloqueó el cierre del resto porque ninguna fila cerrada pasó nunca por
`En curso` mientras dependía de él sin mergear.
...
| 3A-1 | Cerrado | 3A-0b | `claude/ecstatic-hopper-pyo0vy` | [#51](https://github.com/EduardoTerwogt/serenata-erp/pull/51), [#52](https://github.com/EduardoTerwogt/serenata-erp/pull/52), [#53](https://github.com/EduardoTerwogt/serenata-erp/pull/53), [#54](https://github.com/EduardoTerwogt/serenata-erp/pull/54) | `04f091d`/`0279023`/`5b65897`/`2001315` | `be0c8b4`/`5896a4d`/`706f817`/`fadf8bc` | Setup manual completo (proyecto Vercel aislado `serenata-erp-loadtest`, rama `loadtest-target`, 16 env vars, 7 secretos nuevos de GitHub). `workflow_dispatch` real de `load-test.yml` contra Vercel encontró y arregló 3 bugs reales, uno por intento: (1) `LOADTEST_ENV_SECRET` con un LINE SEPARATOR U+2028 colgando (artefacto de copiar/pegar) -- Node no puede meterlo en un header HTTP -- fix `.trim()` en #52; (2) `proxy.ts`/`proxyHandler` bloqueaba `/api/internal/env-check` con 401 antes de que corriera su propio guard -- faltaba en `isPublicPath()` -- fix en #53; (3) `env-check.mjs` concatenaba sin deduplicar los 2 `Set-Cookie` distintos que devuelve `/api/auth/csrf` para `authjs.csrf-token` (uno del middleware, otro de la ruta) -- `MissingCSRF` -- fix `mergeCookies()` en #54, reproducido y confirmado con `next build`+`next start` reales en el sandbox antes de pushear. Un 4to hallazgo, de datos no de código: la cuenta de staff de test no tenía la sección `admin` (caso 5 de la spec funcionando como debía) -- se agregó vía `admin_update_usuario` a `e2e-live@serenata.test`/`e2e-live-local@serenata.test` en `serenata-erp-test`. Run [35023245288](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35023245288): `pin-loadtest-target`/`local`/`serverless` los 3 en verde -- validado end-to-end contra infraestructura real | Arrancar 3A-2 (en paralelo con 3A-3/3A-4) |
| 3A-2 | Cerrado | 3A-1 | `claude/ef3a-fixtures-identidad` | [#55](https://github.com/EduardoTerwogt/serenata-erp/pull/55) | — | `f8c0ec2` | Validación real vía `workflow_dispatch` de `load-test.yml` (`local`) encontró y arregló 2 hallazgos reales, uno por intento: (1) `proxy-handler.ts` bloqueaba `/api/internal/loadtest-portal-session` con 401 antes de su propio guard -- mismo patrón que 3A-1, agregada a `isPublicPath()` junto con `/api/internal/loadtest-drive-folder` (3A-4) de una vez; (2) `prepare-portal-fixtures.mjs` dejaba `portal_estado` en NULL -- `GET /api/portal/me` exige `portal_estado !== null` además de `requirePortalSession()`, fix: proveedores de fixture se crean con `portal_estado: 'activo'`. Los 7 casos de la spec (login REST, creación de identidades/proveedores, cookie real autentica, 4 casos de guard/validación Zod) pasaron en verde | Arrancar 3A-3 |
| 3A-3 | Cerrado | 3A-1 | `claude/ef3a-fixtures-volumen` | [#56](https://github.com/EduardoTerwogt/serenata-erp/pull/56) | — | `41b9ccc` | Validación real vía `workflow_dispatch` de `load-test.yml` (`local`, escala reducida `LOADTEST_PROVEEDORES_TARGET=3`/`LOADTEST_COTIZACIONES_TARGET=3`): 3/3 proveedores insertados, 3/3 cotizaciones creadas+emitidas+aprobadas vía la API real (`POST /api/cotizaciones`+`.../emitir`+`.../aprobar`), conteos post-seed exactos (`proveedores=3`, `cotizaciones=3`, `cuentas_cobrar=3`, `items_cotizacion=15`, `cuentas_pagar=15`) | Arrancar 3A-4 |
| 3A-4 | Cerrado | 3A-1 | `claude/ef3a-cleanup-drive-postgres` | [#57](https://github.com/EduardoTerwogt/serenata-erp/pull/57) | — | `fdaabc0` | Validación real vía `workflow_dispatch` de `load-test.yml` (`local`) encontró y arregló 2 hallazgos reales: (1) faltaba `TEST_SUPABASE_URL` en el env del job `local` -- ningún script plano de 3A-2/3A-3/3A-4 podía correr ahí, solo estaba `NEXT_PUBLIC_SUPABASE_URL`; (2) el script inline de verificación de `bulkCleanupLoadTestRun` se escribía en `/tmp`, fuera del árbol de `node_modules` -- Node no resolvía `@supabase/supabase-js`. Con ambos arreglados: folder real de Drive crear (200+`folderId` real)/borrar (200), guard rechaza secreto incorrecto con 404; cascada de `bulkCleanupLoadTestRun` contra datos sembrados reales (60 cotizaciones×5 items) borró 300 `items_cotizacion`, 300 `cuentas_pagar`, 60 `cuentas_cobrar`/`proyectos`/`cotizaciones`/`proveedores`/`clientes`, 0 residuales. Migración `20260915_loadtest_runs.sql` aplicada a `serenata-erp-test` tras reconectar el MCP de Supabase (caído gran parte de la sesión) -- confirmada con `list_tables` (RLS habilitado, 0 filas) | Arrancar 3A-5 |
| 3A-5 | Cerrado | 3A-1, 3A-2, 3A-3, 3A-4 | `claude/ef3a-k6-escenarios-basicos`/`claude/ef3a-k6-escenarios-avanzados`/`claude/ef3a-k6-telemetria` | [#58](https://github.com/EduardoTerwogt/serenata-erp/pull/58) (a), [#59](https://github.com/EduardoTerwogt/serenata-erp/pull/59) (b), [#60](https://github.com/EduardoTerwogt/serenata-erp/pull/60) (c) | — | `c2959f3` (a), `066b99e` (b), `0ea0c02` (c) | Los 8 escenarios de k6 + telemetría real de `pg_stat_statements` (universo completo, no top-N). Validación real en las 3 PRs vía `workflow_dispatch` de `load-test.yml` (runs [35036940623](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35036940623), [35038537654](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35038537654), [35039625968](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35039625968)) -- incluyó subida real de archivos a Drive, conexión WebSocket real al canal privado de Realtime, y deltas reales de `pg_stat_statements` calculados correctamente contra `serenata-erp-test` | Arrancar 3A-6 |
| 3A-6 | Cerrado | 3A-1, 3A-2, 3A-3, 3A-4, 3A-5 | N/A | N/A | `4550627` | N/A | `docs/archive/ef-3-baseline-previo.md`. Parcial y disclosed: `serverless` bloqueado por rate-limit real de Vercel (retry 24h); `local` reveló fallas masivas (92-99%) bajo concurrencia combinada en 5/7 escenarios (`portal.js` sí 100% limpio a 150 VUs) -- hipótesis de causa raíz: contención de CPU compartida entre la app y los 7 procesos k6 en el mismo runner (diseño heredado de 3A-1). Bug real encontrado: `uploads.js` necesita `setupTimeout` explícito (pendiente, PR normal). Datos huérfanos reales de la corrida limpiados a mano vía Supabase MCP, 0 residuales. Decisión de cierre confirmada por el usuario | Cerrar EF-3A. Pendiente: reintentar 3A-6 cuando Vercel libere el rate-limit |
...
| 3B-7 | Cerrado | 3A-1 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `882e299` | `8dcd02e` | RPC `preview_next_cotizacion_folio_principal()` (migración `20260916_preview_next_cotizacion_folio_principal.sql`) reemplaza el `SELECT id FROM cotizaciones` sin filtro de la rama principal; la rama de complementarias no se toca. Paridad JS-vs-RPC confirmada con 4 casos sintéticos contra `serenata-erp-test`. Migración ya aplicada también a producción (aditiva). PR #61 mergeado a `main` (`8dcd02e`). **F14c encontrado y corregido** durante el sembrado de volumen del Bloque 4: `lpad(n::text, 3, '0')` TRUNCA (no rellena) folios de 4+ dígitos -- con volumen≥1000 la RPC devolvía `'SH100'` en vez de `'SH1000'`, chocando con el folio real #100 (`duplicate key ... cotizacion_folio_reservations_folio_key`), reproducido real al sembrar 995 cotizaciones secuenciales contra `serenata-erp-test`. Afecta también a `reserve_next_cotizacion_folio()` (la RPC atómica pre-existente de 20260408, sin relación con 3B-7 pero mismo patrón de bug) -- ambas corregidas en la misma migración `20260916_fix_folio_lpad_truncation.sql` (`lpad(n::text, GREATEST(3, length(n::text)), '0')`, nunca trunca), aplicada y verificada (`SH1000` correcto) en `serenata-erp-test` y producción. `getNextFolio()` (fallback JS) no tiene este bug -- usa `padStart`, que no trunca. **F14d encontrado y corregido** al reintentar el ciclo real de `load-test.yml`: `SMOKE editar-concurrente.js` chocó contra `'SH2201'` -- una reserva consumida por el test suite `live` (corrida de e2e.yml solapada en el tiempo) cuya cotización fue borrada después por su propio cleanup (`tests/e2e/utils/live-cleanup.ts`, que ya sabía de este hueco pero lo creía exclusivo de test) -- pero `DELETE /api/cotizaciones/[id]` (real, para estado BORRADOR) tiene el mismo efecto en producción. Fix: `reserve_next_cotizacion_folio`/`preview_next_cotizacion_folio_principal` ahora cuentan como ocupado cualquier fila de `cotizacion_folio_reservations` que sobrevive a la purga de abandonadas (consumida, sin importar si la cotización sigue viva, o activa sin expirar) -- migración `20260916_fix_folio_reservation_survives_deletion.sql`, aplicada y verificada (`SH2201` correctamente excluido del preview) en `serenata-erp-test` y producción; preview de complementarias en JS (`folio.ts`) actualizado con el mismo criterio. `SH2201` huérfana limpiada a mano en `serenata-erp-test` (mismo criterio que `cleanupOrphanedFolioReservations()`). **Rate-limit de Vercel de 3A-6 confirmado liberado**: `workflow_dispatch` completo de `load-test.yml` (`pin-loadtest-target`+`local`+`serverless`, run [35137406697](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35137406697)) 100% verde sobre `main` -- deployment READY en 42s, las 8 corridas SMOKE de `local` pasaron limpias (incluida `editar-concurrente.js`, antes rota por F14d). **Gate de p95 medido y superado**: 50 invocaciones directas de `preview_next_cotizacion_folio_principal()` contra `serenata-erp-test` (bracket `clock_timestamp()` dentro de un bloque `DO`, sin red de por medio -- mide el costo real de la RPC en Postgres, que es idéntico sin importar qué cliente la llama) dieron p95=23.27ms (min=15.73, avg=17.48, p50=16.26, max=30.21) -- muy por debajo del umbral de 1000ms. `F14b` **no se activa** (gate superado, no condición de falla). Caché eliminado de `folio.ts` según lo pactado: `CacheManager`/`invalidateFolioCache()` removidos, `previewNextQuotationFolio()` llama la RPC (o la rama de complementarias) directo en cada invocación; único caller de `invalidateFolioCache()` (`approval.ts`, tras aprobar) también limpiado. `npx tsc --noEmit`/`npm run lint` (0 errores)/`npm test` (832/832, 2 tests de caché en `folio.test.ts` reemplazados por 2 que confirman ausencia de caché; `approval.test.ts` sin el mock/assert del caché) en verde | Arrancar el siguiente bloque independiente |
...
| 3B-10 | Cerrado | 3B-1 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `1375290` | `8dcd02e` | 5 RPCs (`dashboard_kpis_cuentas`/`dashboard_egresos_por_bucket`/`dashboard_actividad_cotizaciones`/`dashboard_actividad_proyectos`/`dashboard_cotizaciones_recientes`, migración `20260916_dashboard_agregados_sql.sql`) reemplazan las 4 lecturas de tabla completa de `getResumenDashboard()`. `getPagosComprobantesEnRango()` no se toca (ya filtra server-side, fuera de alcance de F13). De paso elimina el `.limit(500)` real de `getCuentasPagar()` que topaba el KPI "por pagar". `dashboard_egresos_por_bucket` usa `WITH ORDINALITY` (orden validado por separado contra `serenata-erp-test`). 9/9 tests de paridad/resiliencia en verde. Migración aplicada a `serenata-erp-test` y producción (aditiva). PR #61 mergeado (`8dcd02e`) -- criterio de aceptación era solo tests, cierra en el merge | Arrancar el siguiente bloque independiente |
...
| 3C-4 | Cerrado | 3C-3 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61), [#68](https://github.com/EduardoTerwogt/serenata-erp/pull/68) | `be52720` | `8dcd02e` | Alcance base implementado: `app/api/keep-alive/route.ts` con `maxDuration=60`, retención de `rate_limits` (`window_start<24h`, best-effort como `idempotency_keys` pero query distinta) y safety-net de `syncAllDown()` bajo `acquire_sheets_sync_lock`/`renew_sheets_sync_lease`/`release_sheets_sync_lock` (mismo patrón que `app/api/integrations/sheets/sync-down/route.ts`), `triggered_by:'cron:keep-alive'`. 13/13 tests en verde. PR #61 mergeado a `main` (`8dcd02e`). En el camino se encontró y corrigió F14c (ver fila 3B-7). El intento de medir la duración real reveló F15c: `TEST_SHEETS_SPREADSHEET_ID` del entorno aislado de loadtest resultó inaccesible ("Requested entity was not found" de la API de Sheets en las 9 tablas) -- nunca se había ejercitado Sheets contra ese entorno antes. Corregido en PR #68 el gate en sí (ahora también falla si `errors>0`, con detalle por tabla) y agregada herramienta de diagnóstico/corrección (`reset-sheets-integration.mjs`, opt-in) que confirmó -- creando un spreadsheet nuevo con las credenciales reales del entorno -- que la integración funciona (`totalRows=37572 errors=0`) una vez el `spreadsheetId` es el correcto. **Decisión explícita del usuario (2026-09-16): la validación empírica de la duración de `syncAllDown()` contra Sheets queda fuera del alcance de Engineering Hardening** -- qué hacer con la integración de Sheets (spreadsheet de loadtest a reconfigurar, o el futuro de la integración en sí) es una decisión de producto a tomar en otro momento, no un bloqueo técnico de EF-3. No afecta otra funcionalidad: el safety-net ya mergeado es best-effort (try/catch, nunca hace fallar `keep-alive`) y el `spreadsheetId` roto es exclusivo del proyecto Vercel aislado de loadtest, no de producción ni de `serenata-erp-test`. F15b (condicional, gate de duración) **no se evalúa** -- descartado por el mismo alcance, no por resultado. Cierra con retención+safety-net entregados y probados; la medición de duración queda fuera de alcance por decisión de producto, documentada, no deuda oculta | Arrancar el siguiente bloque independiente |
...
| 3D-0b | Cerrado | 3D-0 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | `8e2d1a2` | `2d7bd47` | 13/13 verde, incluye T12 reescrito rojo→verde | Arrancar el siguiente bloque independiente |
| 3D-1 | Cerrado | 3D-0 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | `b2e8255` | `2d7bd47` | `hooks/useQuotationMutationTracker.ts`, T1-T13 verdes | Arrancar el siguiente bloque independiente |
...
| 3D-5 | Cerrado | 3D-0, 3D-1, 3D-2, 3D-3, 3D-4 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61), [#67](https://github.com/EduardoTerwogt/serenata-erp/pull/67) | `62af8de` | `404ce92` | Implementación completa: `hooks/useQuotationItemCellsAutosave.ts` (clúster completo -- autosave de celdas, alta/baja/import de filas, conflicto atómico de 4 campos del autofill de producto, `itemsController`), verbatim desde `page.tsx` (1484→~660 líneas). Alcance ampliado respecto al punto 4 original de la spec (que solo nombraba el autosave puro) para incluir todo el clúster de partidas, misma consistencia que General/Totales/Notas. `resyncPartidas` (de `useQuotationReconciliation`, que depende de los refs de este hook) se pasa vía wrapper estable en un ref para evitar ciclo de imports -- ver comentario junto a `resyncPartidasRef` en `page.tsx`. T4/T5/T6/T9 de `page-autosave-characterization.test.tsx` (3D-0) en verde sin tocar; nuevo `page-item-cells-atomic-conflict.test.tsx` cubre el conflicto atómico multi-campo. **Prueba manual obligatoria completada**: PR #67 agregó el job `test-item-cell-conflict` a `load-test.yml` (opt-in `test_item_cell_conflict=true`), que reutiliza verbatim el escenario "causa F" de `tests/e2e/live/cotizaciones-colaboracion.spec.ts` (PATCH en vuelo sobre la misma celda) apuntando `PLAYWRIGHT_BASE_URL` al deploy real de `serenata-erp-loadtest`. Corrida real ([run 35142008082](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35142008082), job `test-item-cell-conflict`, sobre `main`@`404ce92`): test verde -- los 2 PATCH concurrentes (celdas SH2100/SH2200 durante el sembrado de volumen) devolvieron 200, ninguna edición se perdió y no apareció un 409 contra el propio autor, cumpliendo el criterio de aceptación exacto | Arrancar el siguiente bloque independiente |
...
| 3D-9 | Cerrado | ninguna | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | `6f282d0` | `2d7bd47` | Grep del punto 11 da 2 hits residuales, ambos `const rpcDetail = rpcError.message` (asignación a variable local, usada solo en `logStructured` server-side) en los 2 routes de registrar-pago -- nunca llega a la respuesta al cliente, confirmado por test. El resto de los 5 archivos, incluido el otro patrón (`JSON.stringify(error)`), da 0 | Arrancar el siguiente bloque independiente |
...
| 3E-3 | Cerrado | 3E-2 | N/A | N/A | `321575b` | N/A | `node scripts/validate-ef3-tracker.mjs --require-final --except 3E-3` verde contra este archivo (42 filas parseadas, validación estructural OK, validación --require-final OK) -- ver "## 12. Cierre formal (3E-3)" al final de este documento para el log completo | Cierre de EF-3 |
...
- 40 bloques base, todos `Cerrado`. F27 (índice faltante) se encontró y
  corrigió dentro del propio 3E-1 sin necesitar fila condicional propia --
  se documentó directo en la fila 3E-1, igual que F14c/F14d en 3B-7.
- 2 de 4 filas condicionales posibles activadas: `3D-0b` (F26, cerrada) y
  `3E-1b` (F28, diferida con aprobación) -- `3B-7b`/`3C-4b` nunca se
  activaron (sus gates respectivos, 3B-7 y 3C-4, pasaron sin necesitar el
  bloque correctivo). `3E-1b` se agregó ad hoc vía PR
  [#70](https://github.com/EduardoTerwogt/serenata-erp/pull/70) cuando
  F28 lo exigió, siguiendo la misma regla de proceso de 3A-0b (primero el
  PR que actualiza el validador, después la fila).
