...
| 3C-4 | En curso | 3C-3 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `be52720` | `8dcd02e` | Alcance base implementado: `app/api/keep-alive/route.ts` con `maxDuration=60`, retención de `rate_limits` (`window_start<24h`, best-effort como `idempotency_keys` pero query distinta) y safety-net de `syncAllDown()` bajo `acquire_sheets_sync_lock`/`renew_sheets_sync_lease`/`release_sheets_sync_lock` (mismo patrón que `app/api/integrations/sheets/sync-down/route.ts`), `triggered_by:'cron:keep-alive'`. 13/13 tests en verde. PR #61 mergeado a `main` (`8dcd02e`). **Sigue bloqueado para el gate de duración**: la medición empírica obligatoria exige volumen objetivo (`items_cotizacion≥5500`) sembrado en `serenata-erp-test` -- todavía no ejecutado a escala completa (solo la validación reducida de 3A-3) | Sembrar volumen completo en `serenata-erp-test`, medir duración real de `syncAllDown()` y decidir si activa F15b/3C-4b |
