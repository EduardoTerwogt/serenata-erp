...
| 3B-7 | En curso | 3A-1 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `882e299` | `8dcd02e` | RPC `preview_next_cotizacion_folio_principal()` (migración `20260916_preview_next_cotizacion_folio_principal.sql`) reemplaza el `SELECT id FROM cotizaciones` sin filtro de la rama principal; la rama de complementarias no se toca. Paridad JS-vs-RPC confirmada con 4 casos sintéticos contra `serenata-erp-test`. Migración ya aplicada también a producción (aditiva). PR #61 mergeado a `main` (`8dcd02e`). **Falta medir p95 real sin caché contra `serenata-erp-loadtest`** -- bloqueado por el rate-limit de Vercel de 3A-6 (retry ~24h desde 2026-09-16 ~01:00 UTC, todavía no liberado a las ~09:15 UTC) | Medir p95 real contra `serenata-erp-loadtest` en cuanto el rate-limit de Vercel se libere, y decidir sobre el caché |
