...
| 3C-4 | En curso | 3C-3 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `be52720` | `8dcd02e` | Alcance base implementado: `app/api/keep-alive/route.ts` con `maxDuration=60`, retención de `rate_limits` (`window_start<24h`, best-effort como `idempotency_keys` pero query distinta) y safety-net de `syncAllDown()` bajo `acquire_sheets_sync_lock`/`renew_sheets_sync_lease`/`release_sheets_sync_lock` (mismo patrón que `app/api/integrations/sheets/sync-down/route.ts`), `triggered_by:'cron:keep-alive'`. 13/13 tests en verde. PR #61 mergeado a `main` (`8dcd02e`). **Volumen objetivo ya sembrado**: nuevo job `seed-volumen` en `load-test.yml` (gateado por `seed_volume=true`, [#62](https://github.com/EduardoTerwogt/serenata-erp/pull/62)/[#63](https://github.com/EduardoTerwogt/serenata-erp/pull/63) fix de env vars) corrió a escala completa contra `serenata-erp-test`: proveedores=1,200, cotizaciones=1,200, `items_cotizacion`=6,000 (≥5,500 ✓), `cuentas_pagar`=6,000. En el camino se encontró y corrigió F14c ([#64](https://github.com/EduardoTerwogt/serenata-erp/pull/64) -- ver fila 3B-7). **Sigue bloqueado solo para el gate de duración**: medir `syncAllDown()` real requiere el entorno serverless de `serenata-erp-loadtest`, mismo rate-limit de Vercel de 3A-6 que bloquea 3B-7/3D-5 | Medir duración real de `syncAllDown()` contra `serenata-erp-loadtest` en cuanto el rate-limit de Vercel se libere, y decidir si activa F15b/3C-4b |
...
| 3D-5 | En curso | 3D-0, 3D-1, 3D-2, 3D-3, 3D-4 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | `62af8de` | `8dcd02e` | Implementación completa: `hooks/useQuotationItemCellsAutosave.ts` (clúster completo -- autosave de celdas, alta/baja/import de filas, conflicto atómico de 4 campos del autofill de producto, `itemsController`), verbatim desde `page.tsx` (1484→~660 líneas). Alcance ampliado respecto al punto 4 original de la spec (que solo nombraba el autosave puro) para incluir todo el clúster de partidas, misma consistencia que General/Totales/Notas. `resyncPartidas` (de `useQuotationReconciliation`, que depende de los refs de este hook) se pasa vía wrapper estable en un ref para evitar ciclo de imports -- ver comentario junto a `resyncPartidasRef` en `page.tsx`. T4/T5/T6/T9 de `page-autosave-characterization.test.tsx` (3D-0) en verde sin tocar; nuevo `page-item-cells-atomic-conflict.test.tsx` (aislado por el caché de catálogos de `useQuotationForm.ts`) cubre el conflicto atómico multi-campo, nunca antes probado automatizado. PR #61 mergeado a `main` (`8dcd02e`). **Sigue sin cerrar**: su criterio de aceptación exige una prueba manual documentada contra el entorno serverless real (`serenata-erp-loadtest`), no cierra solo con CI verde | Correr y documentar la prueba manual contra `serenata-erp-loadtest` (editar la misma celda repetidamente con un PATCH en vuelo) |
