...
| 3C-4 | En curso | 3C-3 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61), [#68](https://github.com/EduardoTerwogt/serenata-erp/pull/68) | `be52720` | `8dcd02e` | Alcance base implementado: `app/api/keep-alive/route.ts` con `maxDuration=60`, retención de `rate_limits` (`window_start<24h`, best-effort como `idempotency_keys` pero query distinta) y safety-net de `syncAllDown()` bajo `acquire_sheets_sync_lock`/`renew_sheets_sync_lease`/`release_sheets_sync_lock` (mismo patrón que `app/api/integrations/sheets/sync-down/route.ts`), `triggered_by:'cron:keep-alive'`. 13/13 tests en verde. PR #61 mergeado a `main` (`8dcd02e`). **Volumen objetivo ya sembrado**: job `seed-volumen` en `load-test.yml` corrió a escala completa contra `serenata-erp-test`: proveedores=1,200, cotizaciones=1,200, `items_cotizacion`=6,000 (≥5,500 ✓), `cuentas_pagar`=6,000. En el camino se encontró y corrigió F14c (ver fila 3B-7). **Rate-limit de Vercel de 3A-6 confirmado liberado** (ver fila 3B-7). **Primer intento del gate de duración reveló F15c**: `measure-sync-down-duration.mjs` medía `11.22s` (bajo el umbral de 50s) pero el body real mostraba `totalRows=0 errors=9 state=error` -- el script solo comparaba duración, nunca `errors`/`state`. Causa raíz: `TEST_SHEETS_SPREADSHEET_ID` no es accesible con las credenciales de Google del entorno de loadtest (nunca se había ejercitado antes -- `live` de `e2e.yml` no usa Sheets), error real de Google "Requested entity was not found" en las 9 tablas. Corregido en PR #68: (1) el gate ahora también falla si `errors>0`, con detalle por tabla en el log; (2) `reset-sheets-integration.mjs` (opt-in `reset_sheets_integration=true`) crea un spreadsheet nuevo con las credenciales reales del entorno -- corrida real ([run 35145234157](https://github.com/EduardoTerwogt/serenata-erp/actions/runs/35145234157)) confirmó el spreadsheet nuevo accesible y funcional (sync inicial: `totalRows=37572 errors=0`), id `1muOB02MY1n31xod2mdX2orvmzfOFK5M3fOnph-yuxW0`. **Bloqueado en una acción manual**: ese id nuevo debe guardarse a mano como secreto de GitHub `TEST_SHEETS_SPREADSHEET_ID` y como env var de Vercel (`serenata-erp-loadtest`) `GOOGLE_SHEETS_SPREADSHEET_ID` -- ninguna herramienta disponible en esta sesión puede escribir ninguno de los dos lugares; comunicado al usuario | Usuario actualiza el secreto/env var con el id nuevo; entonces re-disparar `load-test.yml` con `measure_sync_down=true` para la medición real de duración, y decidir si activa F15b/3C-4b |
