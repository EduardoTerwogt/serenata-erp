...
| 3D-0b | Pendiente | 3D-0 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | | Código+tests listos localmente (13/13 verde, incluye T12 reescrito rojo→verde). Ver nota de excepción de proceso arriba |
| 3D-1 | Pendiente | 3D-0 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | | Código+tests listos localmente (`hooks/useQuotationMutationTracker.ts`, T1-T13 verdes). Ver nota de excepción de proceso arriba |
| 3D-2 | Pendiente | 3D-1 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Movió también los tipos/helpers compartidos de General/Totales/Items (antes locales a `page.tsx`) a `lib/quotations/collaboration.ts` nuevo, no listado en el punto 4 de la especificación -- evita un ciclo de imports `page.tsx` <-> `hooks/useQuotationXAutosave.ts` que los 4 hooks de esta sub-secuencia (3D-2..3D-5) habrían necesitado si cada uno reimportara esos tipos desde `page.tsx`. Puramente mecánico (funciones/tipos/clase movidos verbatim), sin cambio de comportamiento -- confirmado por los 13 casos de 3D-0/3D-0b en verde | Código+tests listos localmente (`hooks/useQuotationGeneralAutosave.ts`, `lib/quotations/collaboration.ts`, T1-T13 verdes, `tsc`/build limpios). Ver nota de excepción de proceso arriba |
| 3D-3 | Pendiente | 3D-2 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | | Código+tests listos localmente (`hooks/useQuotationTotalesAutosave.ts`, T1-T13 verdes incluidos T2/T8 de Totales, `tsc`/lint/build limpios). Ver nota de excepción de proceso arriba |
| 3D-4 | Pendiente | 3D-3 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | | Código+tests listos localmente (`hooks/useQuotationNotasAutosave.ts`, T1-T13 verdes, `tsc`/lint/build limpios). Ver nota de excepción de proceso arriba |
| 3D-5 | Pendiente | 3D-0, 3D-1, 3D-2, 3D-3, 3D-4 | — | — | — | — | Pausado por decisión explícita del usuario: además de ser el bloque de mayor riesgo (P0 de implementación, "ya causó bugs reales dos veces"), su propio criterio de aceptación (punto 11) exige una prueba manual contra el entorno serverless real de 3A-1 -- bloqueado por el mismo setup manual de Vercel pendiente que ya bloquea 3A-1/3B-7/3C-4. No cierra solo con CI verde | Retomar cuando 3A-1 esté disponible, o si el usuario decide aceptar el riesgo sin la prueba manual |
| 3D-6 | Pendiente | 3D-0, 3D-0b, 3D-2, 3D-3, 3D-4, 3D-5 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Ejecutado con 3D-5 pausado (decisión del usuario, ver nota de 3D-5): `useQuotationReconciliation` recibe los refs del clúster de partidas (`itemDirtyCellsRef`/`itemFocusedCellsRef`/`itemSavingCellsRef`/`localWriteAtRef`/`pendingRowCreationsRef`) directo de `page.tsx` en vez de desde un hook `useQuotationItemCellsAutosave` -- mismo comportamiento, misma firma interna, distinto origen de esos parámetros. Cuando 3D-5 se retome, esta firma se actualiza para recibirlos de ahí | Código+tests listos localmente (`hooks/useQuotationReconciliation.ts`, T1-T13 verdes incluidos T11/T12/T13 de interleaving, `tsc`/lint/build limpios). Ver nota de excepción de proceso arriba |
| 3D-7 | Pendiente | 3D-0, 3D-2, 3D-3, 3D-4, 3D-5, 3D-6 | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Ejecutado con 3D-5 pausado (ver nota de 3D-5/3D-6): `useQuotationBusinessActions` recibe `flushItemCellDirtyFields` directo de `page.tsx` en vez de un hook `useQuotationItemCellsAutosave`. Asimetría de las 5 acciones (aprobar/generarPDF/generarCotizacion con flush+guard; crearComplementaria sin ninguno; cancelarCotizacion con guard propio sin flush) preservada exactamente, confirmada por lectura del código movido | Código+tests listos localmente (`hooks/useQuotationBusinessActions.ts`, T1-T13 verdes incluido T10 de flush-before-transition, `tsc`/lint/build limpios). Ver nota de excepción de proceso arriba |
| 3D-8 | Pendiente | 3D-0, 3D-1, 3D-2, 3D-3, 3D-4, 3D-5, 3D-6, 3D-7 | N/A | N/A | — | N/A | Bloqueado por diseño hasta que el PR #49 mergee a `main` -- su propia especificación (punto 7) exige 3D-7 "mergeado a main", no solo "listo", porque el `live` completo de este bloque corre contra `main`. No ejecutable dentro de este mismo PR | Retomar como doc-only (2 commits directo a `main`) apenas #49 mergee, con 3D-5 ya resuelto o formalmente diferido |
| 3D-9 | Pendiente | ninguna | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Grep del punto 11 da 2 hits residuales, ambos `const rpcDetail = rpcError.message` (asignación a variable local, usada solo en `logStructured` server-side) en los 2 routes de registrar-pago -- nunca llega a la respuesta al cliente, confirmado por test. El resto de los 5 archivos, incluido el otro patrón (`JSON.stringify(error)`), da 0 | Código+tests listos localmente (`cancellation.ts` + 4 rutas migradas a `DomainError`/`buildErrorResponse`, 9 tests nuevos/extendidos, `tsc`/lint/build limpios). Ver nota de excepción de proceso arriba |
| 3D-10 | Pendiente | ninguna | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Migradas las 8 rutas de Portal (login, signup, signup/confirmar, documentos GET+POST, cuentas GET, cuentas/[id]/factura POST, me GET, perfil GET+PATCH) de `toErrorMessage`+`Response.json` manual a `buildErrorResponse`. `toErrorMessage`/`error-message.ts` quedan intactos (no se tocan, siguen usados en otros lugares) tal como exige el punto 5 de la especificación. `perfil` usa GET+PATCH en el código real (la especificación decía "GET+PUT") -- se siguió el código, no el texto de la spec | Código+tests listos localmente: 34 tests preexistentes sin cambios de comportamiento + 8 tests nuevos de fuga (uno por ruta, Postgres crudo -> 500 genérico+requestId), más 2 test files nuevos (`portal-cuentas-route.test.ts`, `portal-me-route.test.ts`) y extensión de `portal-perfil-route.test.ts` que no existían antes. Suite completa 758/758, `tsc`/lint/build limpios (build falla en el mismo punto preexistente y no relacionado: `supabaseUrl is required` en `/api/admin/usuarios/[id]`, sandbox sin env vars). Ver nota de excepción de proceso arriba |
| 3D-11 | Pendiente | ninguna | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Migradas `app/api/proyectos/[id]/route.ts` (PUT, único catch, sin mapeo de dominio previo) y `app/api/proyectos/[id]/tipo/route.ts` (PUT, preserva intacto el mapeo `TipoYaAsignadoError` -> 409 antes del catch genérico) a `buildErrorResponse`. `GET /api/proyectos/[id]` no se toca (fuera de alcance, su 404 propio no es el catch genérico) | Código+tests listos localmente: `proyectos-detail-route.test.ts` extendido con el caso de error crudo genérico (sin ningún caso de `TipoYaAsignadoError`, esa ruta nunca lo lanza), `proyectos-tipo-route.test.ts` nuevo (409 preservado + fuga cerrada). Suite completa 763/763, `tsc`/lint limpios, build falla en el mismo punto preexistente y no relacionado (`supabaseUrl is required`, sandbox sin env vars). Ver nota de excepción de proceso arriba |
| 3D-12 | Pendiente | ninguna | `claude/nifty-hypatia-n1tptj` | [#49](https://github.com/EduardoTerwogt/serenata-erp/pull/49) | — | — | Nuevo `lib/server/uploads/factura-validation.ts` (`validateFacturaFiles`) consolida required/tipo-MIME-o-extensión/tamaño para CxP (`pdfRequired: true`), CxC (`pdfRequired: false`, nunca emite `PDF_REQUIRED`) y Portal (`pdfRequired: true`). Cada ruta conserva su propia tabla de mensajes exacta (difieren entre las 3, confirmado leyendo los 3 archivos) mapeando el código devuelto. El fallback por extensión y el chequeo de contenido XML (`.trim().startsWith('<')`, fuera de alcance de este módulo) se preservan intactos en cada ruta. Magic-byte validation deliberadamente fuera de alcance (nunca fue un hallazgo confirmado) | Código+tests listos localmente: nuevo `lib/server/uploads/__tests__/factura-validation.test.ts` (14 tests unitarios del módulo) + las 14 combinaciones código×ruta aplicables (5 CxP + 4 CxC + 5 Portal) agregadas a los 3 test files de ruta existentes, sin tocar sus aserciones previas. Suite completa 792/792, `tsc`/lint limpios, build falla en el mismo punto preexistente y no relacionado (`supabaseUrl is required`, sandbox sin env vars). Ver nota de excepción de proceso arriba |
