...
| 3C-4 | Pendiente | 3C-3 | `claude/brave-maxwell-a2j60v` | [#61](https://github.com/EduardoTerwogt/serenata-erp/pull/61) | — | — | Alcance base implementado: `app/api/keep-alive/route.ts` con `maxDuration=60`, retención de `rate_limits` (`window_start<24h`, best-effort como `idempotency_keys` pero query distinta) y safety-net de `syncAllDown()` bajo `acquire_sheets_sync_lock`/`renew_sheets_sync_lease`/`release_sheets_sync_lock` (mismo patrón que `app/api/integrations/sheets/sync-down/route.ts`), `triggered_by:'cron:keep-alive'`. 13/13 tests en verde. **Sigue bloqueado para el gate de duración**: la medición empírica obligatoria exige volumen objetivo (`items_cotizacion≥5500`) sembrado en `serenata-erp-test` -- todavía no ejecutado a escala completa (solo la validación reducida de 3A-3) | Mergear PR #61, sembrar volumen completo, medir duración real de `syncAllDown()` y decidir si activa F15b/3C-4b |
