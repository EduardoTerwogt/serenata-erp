...
## Tabla de auditoría — ronda 12

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | La Sección 1 seguía diciendo "décima versión" pese a que la propia tabla de ronda 11 (ítem 6) declaraba haberla corregido a "décima primera" — el texto real del cuerpo nunca se actualizó, solo la tabla de auditoría lo prometía | Corregido el cuerpo real de la Sección 1 a "décima segunda versión" (refleja que este documento ya es la v12, ronda 12) | Sección 1 |
| 2 | El "historial de cambios de 8 líneas" que 3A-0 debe escribir en el canónico quedó fijado en la ronda 9 (cuando el plan tenía 8 rondas reales) y nunca se actualizó en las rondas 10, 11 y 12 — un historial de "8 líneas" hoy omitiría las rondas 9, 10, 11 y 12, que contienen hallazgos reales (RLS, distinción lease-perdido-vs-fallo-de-RPC, fix de `Date.now()`, `SMOKE` fixtureCount, F26/3D-0b, y las 6 correcciones de esta ronda) | Corregido a un historial de una línea por cada ronda real (hoy 12), sin fijar un número absoluto que vuelva a quedar obsoleto en la próxima ronda | 3A-0 |
| 3 | 3A-1 declaraba "11 casos" en los puntos 10/11/13/15, pero el propio punto 10 enumera 14 casos distintos (6 de `env-check`, 1 de concurrency, 1 de SHA fijado, 1 del guard de ancestro, y 5 sub-casos de `wait-for-deployment.mjs` incluido el caso nuevo de `serverless` esperando a `local`) — verificado por conteo directo de las cláusulas del propio punto 10, no arrastrado | Unificado a "14 casos" en los 4 lugares que decían "11" (3A-2 con "7 casos" se verificó aparte y está correcto, sin cambios) | 3A-1 |
| 4 | 3C-4 describía el cleanup existente de `idempotency_keys` como `DELETE ... WHERE window_start < now() - interval '24 hours'`, pero la query real (`app/api/keep-alive/route.ts:39-56`, verificado contra el código) filtra por `created_at < now() - interval '7 days' AND status_code IS NOT NULL` — `window_start` es una columna de `rate_limits`, no de `idempotency_keys`. El SQL nuevo de `rate_limits` en sí (que sí usa `window_start`, columna real de esa tabla) no cambia — solo la frase de contexto que describe el precedente | Corregida la descripción del precedente de `idempotency_keys` a la query real (`created_at`, 7 días, `status_code IS NOT NULL`) | 3C-4 |
| 5 | 3B-4 afirmaba "2 consumidores: `app/cotizaciones/page.tsx` (lista filtrada simple) y `QuotationCopyItemsModal.tsx`" para `getCotizaciones()`/`fetchQuotationsList()` — verificado por grep exhaustivo que **`page.tsx` no llama `fetchQuotationsList()`**, hace un `fetch('/api/cotizaciones')` propio directo; el único caller real de `fetchQuotationsList()` es el modal | Corregida la descripción: `page.tsx` consume `GET /api/cotizaciones` directo (no vía ese helper); el trabajo planeado (paginar `page.tsx` vía la RPC/ruta nueva, rediseñar el modal en 2 pasos, eliminar `fetchQuotationsList()`) no cambia, solo la descripción de cómo `page.tsx` obtiene los datos hoy | 3B-4 |
| 6 | 3B-6 afirmaba "6 consumidores confirmados" de `getProveedores()`, listando `app/proveedores/page.tsx`, `TareaFormModal.tsx`, `quotation-service.ts::fetchProveedores()`, `plantillas-servicios/nueva`, `plantillas-servicios/[id]/editar`, y `generar-hoja-llamado/route.ts` — verificado por grep exhaustivo que solo **3** de esos son callers reales de la función de repositorio (`documentos-resumen/route.ts`, `proveedores/route.ts`, `generar-hoja-llamado/route.ts`); los otros 4 son componentes cliente que consumen `/api/proveedores` por HTTP, nunca pueden llamar la función de repositorio directo (corren en el browser) | Corregida la lista a los 3 callers reales de la función de repositorio; aclarado que los componentes cliente la alcanzan de forma transitiva vía `app/api/proveedores/route.ts` (uno de los 3 reales) — el fix de paginación por keyset sigue siendo correcto y necesario sin cambios | 3B-6 |

## Tabla de auditoría — ronda 11

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | La métrica `realtime_propagation_ms` restaba `performance.now_recibido - timestamp_envio` — `performance.now()` mide tiempo monotónico relativo al origen del proceso, no comparable con un timestamp de época; el resultado sería inválido o negativo | Corregido a `Date.now()` en ambos lados de la resta (envío y recepción) | 3A-5 |
| 2 | `create-drive-run-folder.mjs` (Node plano) pretendía llamar `createDriveFolder()` directo, una función de `drive.ts` (TypeScript dentro de la app Next.js) — mismo problema ya resuelto en 3A-2 para `signPortalSession`, repetido aquí sin la misma solución | Mismo patrón que 3A-2: nuevo endpoint interno `POST /api/internal/loadtest-drive-folder`, guardado fail-closed + validado con Zod (`LoadtestDriveFolderSchema`), que sí puede importar `createDriveFolder`; el script `.mjs` lo llama por HTTP y escribe `loadtest_runs` vía `@supabase/supabase-js` directo | 3A-4 |
| 3 | `SMOKE=1` solo cambia `options.scenarios`/`vus`/`iterations` — el `setup()` de `uploads.js` corre siempre completo sin importar ese modo, así que una corrida de humo igual habría creado+emitido+aprobado 50 cotizaciones antes de su única iteración | `setup()` de `uploads.js` ahora calcula `fixtureCount = __ENV.SMOKE === '1' ? 1 : 50`; confirmado que `editar-concurrente.js` ya era SMOKE-seguro por construcción (siempre crea 1, sin importar VUs) | 3A-5 |
| 4 | 3E-2 excluía solo `3E-3` de su verificación de "filas restantes" (39), pero esas 39 seguían incluyendo a la propia `3E-2` — la misma auto-referencia imposible que ya se había resuelto para `3E-3` | Corregido a 38 filas base (excluye `3E-2` y `3E-3`); `3E-2` cierra su propia fila como parte de su propio commit, sin necesitar verificarse a sí misma externamente | 3E-2 |
| 5 | El criterio de cleanup de 3A-4 decía "9 tablas de negocio cubiertas" pero enumeraba 11 (arrastrado de antes de que `clientes`/`productos`/`historial_responsable` se agregaran) | Corregido uniformemente a "11 tablas" | 3A-4 |
| 6 | La sección de contexto decía "novena versión" pese a que el documento ya es v10/ronda 10 (arrastrado) | Corregido a "décima versión" (y ahora "décima primera" en esta ronda) | Sección 1 |
| 7 | Conteos de pruebas/aceptación/evidencia inconsistentes dentro del mismo bloque: 3A-1 decía "11 casos" en el criterio de aceptación pero "10 casos" en la evidencia; 3A-2 decía "7 casos" en el criterio de aceptación pero "4 casos" en la evidencia | Unificado un solo conteo por bloque, repetido igual en pruebas/aceptación/evidencia (11 en 3A-1, 7 en 3A-2) | 3A-1, 3A-2 |
| 8 | 3A-6 seguía describiendo los escenarios como "registran pagos", contradiciendo la decisión explícita de 3A-4 ("la suite de carga no registra pagos" — ningún escenario llama una ruta `.../registrar-pago`) | Corregido a "editan partidas" con nota explícita remitiendo al inventario cerrado de 3A-4 | 3A-6 |

## Tabla de auditoría — ronda 10

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | `editar-concurrente.js` pretendía publicar un `id` creado en runtime vía `SharedArray`/`open()` — ambas APIs de k6 solo leen archivos preparados en la fase de inicialización, no pueden contener datos de un `POST` HTTP hecho en runtime; y proponía "1 sola conexión Realtime compartida entre las 10 VUs", imposible porque cada VU de k6 es un runtime aislado sin memoria compartida | Reescrito con `export function setup()` (ciclo de vida real de k6) que crea la cotización/ítem una vez y los pasa a cada VU vía su valor de retorno; la conexión Realtime pasa a un escenario observador dedicado de 1 VU, correlacionando eventos por el propio `mutation_id` (sin estado compartido entre VUs) | 3A-5 |
| 2 | El evento Realtime seguía abierto como "`general_confirmed` u otro equivalente, a confirmar al implementar", pese a que el PATCH de partida real emite `item_confirmed` con `mutation_id` — y la medición de latencia se proponía contra la respuesta HTTP, que puede resolver después del broadcast | Cerrado contra el código real (`items/[itemId]/route.ts:114-127`, `broadcast.test.ts:25`): evento `item_confirmed`, payload exacto, correlación por `mutation_id` con el timestamp de envío codificado en el propio string (capturado antes del `fetch`, no en la respuesta HTTP); agregado el manejo de `heartbeat`/`phx_reply` de Phoenix Channels para que la conexión observadora no se caiga a mitad de la meseta | 3A-5 |
| 3 | `uploads.js` mandaba `x-loadtest-drive-folder-id: loadtest-${runId}` — un nombre, no un ID real de Drive; `resolveUploadFolderId`/`uploadFileToDrive` usan ese header directo como ID de carpeta padre de la API de Drive (que no resuelve nombres), así que cada subida habría fallado con 404 real | Nuevo script `create-drive-run-folder.mjs` crea la carpeta real vía `createDriveFolder()` antes de que arranque k6, guarda el ID real en `loadtest_runs.drive_folder_id`, y lo pasa a k6 vía `--env LOADTEST_DRIVE_FOLDER_ID=<id-real>` — `uploads.js` manda ese ID real, nunca el nombre | 3A-4, 3A-5 |
| 4 | El fixture de `crear-cotizaciones.js` omitía `x_pagar`; `approve_cotizacion` no crea ninguna fila en `cuentas_pagar` para un ítem con `x_pagar<=0` (confirmado en la migración real) — `cuentas.js`/`uploads.js` se habrían quedado sin ningún objetivo real | Agregado `x_pagar: 700` (y `responsable_id` de un proveedor real de fixture) al payload base; `uploads.js` además crea sus propias cuentas en su propio `setup()`, con `x_pagar>0`, sin depender de otro escenario | 3A-5 |
| 5 | `wait-for-deployment.mjs` se invocaba con `--project-id <id>` sin que existiera ningún secreto/variable de origen para ese ID; los jobs `local`/`serverless` de `load-test.yml` no tenían ninguna relación `needs:` entre sí — `concurrency` a nivel de workflow solo serializa corridas distintas, nunca jobs dentro de la misma corrida, así que ambos habrían mutado `serenata-erp-test`/Drive de test en paralelo | Agregado el secreto `LOADTEST_VERCEL_PROJECT_ID` (y `LOADTEST_VERCEL_TEAM_ID` opcional) a la tabla de variables y a la llamada real del script; agregado `serverless: needs: [pin-loadtest-target, local]` para forzar secuencia | 3A-1 |
| 6 | 3C-2 (su propio PR) llamaba `assertHeartbeatOk(onHeartbeat)`, pero esa función y el tipo `SyncHeartbeat` se definen recién en 3C-3 — si 3C-2 se mergeaba antes que 3C-3 (el grafo los declara secuenciales, PRs separados), el código no compilaría | 3C-2 implementa únicamente el keyset, sin ningún parámetro de heartbeat; 3C-3 es quien, en su propio PR posterior, le agrega el parámetro `onHeartbeat`/tipo `SyncHeartbeat` a la función ya mergeada | 3C-2, 3C-3 |
| 7 | La promesa de "paridad exacta" entre JS (`toFixed(2)`) y Postgres (`ROUND(numeric,2)`) es matemáticamente falsa para valores como `100.005` — `(100.005).toFixed(2)` da `"100.00"` en JS por el error de representación en punto flotante, mientras `ROUND(100.005,2)` da `100.01` en Postgres sobre el decimal exacto; el propio caso de prueba de la ronda anterior asumía que ambos coincidían | Fijada una política canónica única (Postgres/`numeric`/`ROUND`); nueva función `round2` decimal-segura (parsea el string, no el float) en `lib/server/shared/decimal.ts`, que consolida (no duplica) el `round2` ya existente y con el mismo bug en `dashboard.ts:100` y el `toFixed` de `status.ts`; nuevo caso de prueba que documenta la divergencia real antes del fix y su corrección después | 3B-1, 3B-10 |
| 8 | 3B-2 y 3B-4 dejaban SQL fragmentado (`SELECT ... FROM cotizaciones c`, `WHERE ...`) en vez de la función completa; 3B-4 mezclaba `itemsCount`/`items_count` sin decir dónde ocurre la conversión | Escritas las 2 RPCs completas (`buscar_cuentas_cobrar`, `buscar_cotizaciones`) con clamp, rows, totales/conteos, orden y `REVOKE`/`GRANT`, mismo patrón que 3B-3; documentado que la RPC devuelve `items_count` (snake_case) y `quotation-service.ts::fetchQuotationsPage` lo mapea explícitamente a `itemsCount` (camelCase) antes de que el modal lo vea | 3B-2, 3B-4 |
| 9 | `dashboard_cotizaciones_recientes()` y `buscar_ordenes_pago()` usaban `jsonb_agg(t)` sin `ORDER BY` dentro del propio agregado — un agregado de Postgres no hereda el orden de su subconsulta sin repetir el `ORDER BY` a su propio nivel, dejando el orden de salida indefinido | Corregidas ambas a `jsonb_agg(t ORDER BY ...)`, repitiendo el orden real (`created_at DESC, id DESC` / `fecha_generacion DESC, id DESC`) en el nivel del agregado | 3B-10, 3B-11 |
| 10 | El paso (b) de la remediación de 3C-4 (`CHUNK_TABLES`) solo particiona por tabla completa — si una única tabla (`items_cotizacion` al volumen objetivo) por sí sola ya excede el margen de `maxDuration`, bajar `CHUNK_TABLES` a 1 no resuelve nada | Agregado un paso (c): reanudación por cursor de página DENTRO de una tabla (`next_page_cursor`), activado solo si (b) tampoco alcanza — ninguna tabla, sin importar su volumen, puede exceder el margen | 3C-4 |
| 11 | 3A-4 afirmaba que ningún caller pasa `p_operation_id` a `registrar_pago_cuenta_pagar`/`_cobrar` (tabla `pago_operations` "inerte") — falso: ambas rutas reales SÍ lo pasan vía `withIdempotency`, confirmado leyendo el código; la razón real para excluir la tabla del cleanup por `runId` es otra | Corregido: `pago_operations` es real y usada, pero **ningún escenario de la suite de carga registra pagos** (confirmado contra la especificación exhaustiva de los 8 escenarios) — por eso queda fuera del cleanup, no por estar inerte | 3A-4 |
| 12 | 3D-3 describía el conflicto atómico multi-campo de Totales como `descripcion`/`categoria`/`precio_unitario`/`x_pagar` — esos son campos reales de **partidas** (`items_cotizacion`), confirmado en `page.tsx:138-235`; los campos reales de Totales son `porcentaje_fee`/`iva_activo`/`descuento_tipo`/`descuento_valor` (`page.tsx:58-61,260-263`) | 3D-3 corregido a sus 4 campos reales; el grupo de partidas movido a 3D-5 (que sí lo posee), con su propio caso de prueba | 3D-3, 3D-5 |

**Tracker y cierre:**

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 13 | La matriz reserva `F26` para "3D-0 revela una carrera real", pero 3D-0 (punto 2) ordenaba crear un `F27` nuevo en ese caso — contradicción directa, `F26` ya existe para exactamente esa situación | 3D-0 activa `F26` (ya reservado), nunca un ID nuevo espontáneo | 3D-0 |
| 14 | 3D-0 tiene 12 casos (T1-T12), pero 3D-1/3D-2/3D-3/3D-4 seguían diciendo "los 7 casos de 3D-0" (arrastrado de una versión anterior con menos casos) | Las 5 menciones corregidas a "los 12 casos (T1-T12) de 3D-0" | 3D-1, 3D-2, 3D-3, 3D-4 |
| 15 | Hay 6 bloques doc-only reales (3A-0, 3A-6, 3D-8, 3E-1, 3E-2, 3E-3), pero 3A-0 (punto 2, regla uniforme) y la tabla de campos del tracker seguían diciendo "4"; 3D-8 se autodescribía como "7mo" (matemáticamente inconsistente con una lista de 6) | Corregido a "6 bloques doc-only" en ambos lugares, listando los 6 IDs; 3D-8 corregido a "el 6to de los 6" | 3A-0, 3D-8 |
| 16 | Un merge commit no puede llevar escrito su propio SHA (mismo motivo que ya se resolvió para los bloques doc-only), pero el paso 6 de "Publicación de En curso" describía "el propio merge... actualiza la fila con el SHA real del merge commit" como si fuera posible | Reescrito: el merge deja la fila pendiente/sin el SHA de merge; un commit de sincronización doc-only inmediato posterior, directo a `main`, es quien llena `Commit de merge` con el hash ya existente — mismo mecanismo de 2 pasos que los bloques doc-only, aplicado también a los bloques con rama | 3A-0 |
| 17 | El validador de 3A-0b (3 rutas: estructura de tracker) no tiene ningún modo para verificar "todas las filas terminaron" — pero 3E-3 pretendía usarlo para exactamente eso, y 3E-2 exigía que las 40 filas base (incluida la propia `3E-3`, que corre después) ya estuvieran terminadas antes de que 3E-2 mismo cerrara — imposible de cumplir sobre sí mismo | Nuevo modo `--require-final [--except <ID>]` en el validador de 3A-0b, con sus propios casos de prueba; 3E-2 exige ahora las 39 filas base restantes (nunca la propia 3E-3); 3E-3 se invoca con `--require-final --except 3E-3` | 3A-0b, 3E-2, 3E-3 |
| 18 | Las reglas de cierre (sección 6/7/9) sí permiten `Diferido con aprobación` como estado terminal legítimo de una fila, pero 3E-3 (punto 2/10) solo aceptaba `Cerrado`/`No aplica` — una fila legítimamente diferida con aprobación habría hecho fallar la verificación final sin motivo | 3E-3 acepta ahora los 3 estados terminales (`Cerrado`/`No aplica`/`Diferido con aprobación` con nota de aprobación no vacía), vía el mismo modo `--require-final` que ya valida esa regla de 3 estados | 3A-0b, 3E-3 |
| 19 | Correcciones de consistencia menores: "octava versión" → "novena" en la sección 1 (arrastrado); inventario de la sección 3 seguía diciendo "Único" para `getCotizaciones()` (5 consumidores) y "5 consumidores/orden en Node" para `getProveedores()` (6 consumidores/orden por RPC), pese a que ambos cuerpos ya reflejaban los números reales; 3D-12 prometía "15 combinaciones código×ruta" cuando `PDF_REQUIRED` no aplica a CxC (14 aplicables); 3B-6 seguía con "si lo hay" para el test de `generar-hoja-llamado`, pese a estar confirmado por `Glob` que no existe | Las 5 correcciones aplicadas en sus ubicaciones exactas | Sección 1, sección 3, 3D-12, 3B-6 |

## Tabla de auditoría — ronda 9

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | 3A-1: tabla de variables incompleta (faltaban `AUTH_TRUST_HOST`/`NEXTAUTH_URL`/`CRON_SECRET` de runtime y `PLAYWRIGHT_TEST_EMAIL`/`PASSWORD` de scripts); permiso `contents: write` puesto a nivel de workflow en vez de job; espera de deployment de Vercel sin mecanismo concreto (sin estados/timeout) | Tabla dividida en 2 (runtime de la app vs. scripts/workflow) con las 5 variables agregadas y su origen exacto; `permissions` movido dentro del job `pin-loadtest-target`; `scripts/loadtest/wait-for-deployment.mjs` nuevo, con polling cada 5s, estados terminales `READY`/`ERROR`/`CANCELED` y timeout de 300s, ambos casos de falla abortando el job | 3A-1 |
| 2 | 3A-2: el nuevo endpoint interno de sesión de Portal no tenía validación Zod, violando la regla obligatoria de `.claude/rules/api.md` | Agregado `LoadtestPortalSessionSchema` (uuid + entero positivo) y el patrón `validate()` estándar antes de llamar `signPortalSession`; 3 casos de test nuevos (UUID inválido, `sessionVersion` no entero/≤0, body no JSON) | 3A-2 |
| 3 | 3A-1/3A-2: `env-check` solo verificaba `LOADTEST_MODE`/secreto por separado (no el AND explícito) y no confirmaba que la cuenta de staff de `serenata-erp-test` tuviera permiso `admin` para crear los 10 usuarios efímeros que 3A-2 necesita | La ruta ahora exige `LOADTEST_MODE==='true' AND` secreto correcto en un solo `if`; `env-check.mjs` agrega un `GET /api/admin/usuarios` con la sesión de staff y aborta con mensaje explícito si no da 200 | 3A-1 |
| 4 | 3A-4: `resolveUploadFolderId` fallaba abierto (devolvía el folder normal) cuando llegaban headers de override pero `LOADTEST_MODE` estaba apagado — una corrida mal dirigida podría subir archivos reales al folder normal en silencio; `discoverIdsByPrefix`/`discoverIdsWhereIn` paginaban con `.range()` sin `.order()`, sin garantía de orden estable; inventario de cleanup incompleto (`clientes`/`productos`/`historial_responsable` sin cubrir pese a tener escritores reales confirmados); el criterio de cierre "0 filas con runId" no excluía la fila de auditoría de `loadtest_runs` que el propio bloque decide conservar | Cualquier header de override presente ahora exige `LOADTEST_MODE==='true'` Y secreto correcto sin excepción, si no lanza `LoadtestOverrideRejectedError`; `discoverIdsByPrefix`/`discoverIdsWhereIn` agregan `.order('id', {ascending:true})` + cursor real por `id`; `bulkCleanupLoadTestRun` agrega limpieza de `clientes`/`productos`/`historial_responsable` con su propia prueba dedicada; criterio de cierre reescrito para excluir explícitamente la fila de auditoría conservada de `loadtest_runs` | 3A-4 |
| 5 | Suite de carga (3A-5) incompleta: solo había nombres de archivo y parámetros de ejecución, sin especificar qué hace cada escenario (endpoints, payloads, fixtures, métricas, filas creadas, cleanup) — dos implementadores habrían construido pruebas distintas | Especificación exhaustiva punto por punto de los 8 escenarios (incluidos los 2 archivos de fixture nuevos para `uploads.js` y el handshake real de autorización de Realtime en 2 pasos para `editar-concurrente.js`, verificado contra `lib/realtime/authorize.ts` real) | 3A-5 |
| 6 | 3B-4: se declaraba un único consumidor de `getCotizaciones()`, pero `QuotationCopyItemsModal.tsx` (confirmado leyendo el archivo completo) también carga todas las cotizaciones completas y lee `items` en memoria — paginar el endpoint sin rediseñar el modal lo habría roto para cotizaciones fuera de la primera página | Rediseño explícito del modal en 2 pasos: lista paginada server-side (reutilizando la misma RPC) + `fetchQuotationDetail(id)` (ya existente) al seleccionar; nuevo caso de prueba con una cotización fuera de la primera página | 3B-4 |
| 7 | 3B-3: la RPC de CxP usaba `cp.*` bare, perdiendo el fallback real de `proyecto_nombre` (`row.proyecto_nombre \|\| cotizaciones.proyecto \|\| proyectos.proyecto`) que el repositorio JS actual sí aplica; además un `jsonb_agg(t)` sobre la fila completa arrastraba columnas auxiliares de orden al JSON final | Agregado `LEFT JOIN cotizaciones`/`proyectos` + `COALESCE` en las 3 consultas SQL (página, conteo, totales), con caso de prueba dedicado; `jsonb_agg` corregido a agregar solo `t.row_json` con `ORDER BY` explícito repetido en el nivel externo | 3B-3 |
| 8 | 3B-6: el comparador de la ronda 8 ("ajustar `localeCompare` hasta que coincida con Postgres") dejaba abierto si la collation de Node coincidía con la de Postgres — no era una especificación cerrada | Reemplazado por una RPC parametrizada (`proveedores_pagina_por_nombre`) que compara `(nombre, id) > (cursor)` como tupla nativa de Postgres, dejando que el propio `ORDER BY` de la base decida el orden — sin reimplementación en otro lenguaje; agregado el 6to consumidor confirmado (`generar-hoja-llamado/route.ts`) | 3B-6 |
| 9 | 3B-10: `jsonb_array_elements()` sin `WITH ORDINALITY` no garantiza que el orden de salida respete el orden del array de entrada, pero el código JS interpreta el último bucket como el periodo vigente | Agregado `WITH ORDINALITY` + `ORDER BY` la posición ordinal en el `jsonb_agg` externo, con caso de prueba dedicado sobre ≥4 buckets verificando orden posición por posición | 3B-10 |
| 10 | 3B-12: ruta de test declarada con una ubicación bajo `tests/e2e/...` que no corresponde al patrón real del repo | Corregida a la ruta real `app/api/__tests__/proveedores-documentos-resumen-route.test.ts` | 3B-12 |
| 11 | 3C-1: conteo de llamadas/archivos de `triggerSheetsSync()` impreciso ("29 llamadas en 26 rutas") | Verificado por grep exhaustivo: 31 llamadas reales en 28 archivos (3 con 2 llamadas cada uno) — la lista de 28 archivos ya estaba completa, solo el resumen numérico estaba mal | 3C-1 |
| 12 | 3C-3: `sheets_sync_status` se creaba sin RLS, inconsistente con el resto de tablas del plan; `release_sheets_sync_lock` no validaba su `p_state` ni limpiaba `lease_expires_at`; `acquire_sheets_sync_lock` no reiniciaba `rows_synced`/`tables_failed` en una adquisición nueva (métricas de la corrida anterior podían filtrarse); el heartbeat ignoraba `error` de la llamada RPC de renovación, confundiendo un fallo real de infraestructura con un lease genuinamente perdido | RLS habilitado + `REVOKE`/`GRANT service_role`; `release` valida `p_state IN ('idle','error')` y limpia `lease_expires_at`; `acquire` reinicia ambas métricas al adquirir; heartbeat ahora relanza el `error` de la RPC en vez de convertirlo en `false`, distinguiendo explícitamente "lease perdido" (409) de "fallo real de RPC" (500/fallo de tabla, nunca 409); catch de `syncTableDown` corregido para relanzar `SheetsSyncLeaseLostError` antes de su conversión genérica a `{ok:false}` | 3C-3 |
| 13 | Sistema de tracker/cierre con 6 fallas de diseño: el validador solo leía la ruta activa del canónico, rompiéndose después de que 3E-2 archiva el documento; `tracker-lint` se describía como condicionado por path de archivo, algo que GitHub Actions no soporta de forma confiable a nivel de job; 3E-3 no tenía ningún entregable/mecanismo de commit real para cerrarse a sí mismo; solo 4 de los 6 bloques doc-only reales estaban listados (faltaban 3D-8 y 3E-3); el criterio de cierre exigía uniformemente "PR+merge SHA", que no aplica a un bloque doc-only; no había regla que impidiera que un hallazgo nuevo agregara una fila espontánea al tracker sin pasar primero por el validador/la matriz; el historial de versiones solo cubría las rondas 1-5 pese a que el documento ya iba en la ronda 8+ | Validador prueba 2 rutas (activa y archivada) en orden, fallando explícito si ninguna existe; `tracker-lint` corre siempre en cada push/PR (sin condicionar por path, justificado explícitamente); 3E-3 rediseñado con un entregable real (sección final de cierre en el archivo archivado + corrida de `validate-ef3-tracker.mjs` documentada) y el mismo mecanismo de 2 commits; los 6 bloques doc-only (3A-0, 3A-6, 3D-8, 3E-1, 3E-2, 3E-3) listados explícitamente en todos los puntos relevantes; el criterio de cierre distingue "PR+merge SHA" (bloques con rama) de "Commit SHA" (doc-only); regla de proceso explícita: un hallazgo nuevo primero actualiza matriz/grafo/validador vía su propio PR, y solo después se agrega la fila del tracker; historial de versiones extendido a las 8 rondas reales | 3A-0b, 3D-8, 3E-3, sección 2, sección 4, sección 7, sección 9 |

## Tabla de auditoría — ronda 8

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | `bulk-cleanup.mjs` (3A-4) llamaba 7 veces a `deleteByIdsInChunks()`, pero esa función se había renombrado a `deleteWhereInChunks()` en la ronda 7 al corregir el bug de reservas — la definición original quedó eliminada, el script fallaría al ejecutarse | `deleteByIdsInChunks` reincorporada como envoltorio delgado sobre `deleteWhereInChunks(supabaseAdmin, table, 'id', ids)` — las 7 llamadas existentes vuelven a resolver, la única llamada por columna explícita (`folio`) sigue usando `deleteWhereInChunks` directo | 3A-4 |
| 2 | 3A-4 afirmaba que las 3 rutas de subida cierran su catch con `buildErrorResponse`, pero `app/api/portal/cuentas/[id]/factura/route.ts` (confirmado leyendo el archivo real) usa un catch crudo con `toErrorMessage`+500 — `LoadtestOverrideRejectedError` no habría producido el 400 prometido en esa ruta | Agregado un `instanceof LoadtestOverrideRejectedError` explícito antes del catch crudo, solo en esa ruta; anotado en 3D-10 (que migra esa misma ruta a `buildErrorResponse` más adelante) para que elimine ese `instanceof` como parte de su propia migración, sin dejarlo como código muerto | 3A-4, 3D-10 |
| 3 | El validador de 3A-0b ya admitía IDs condicionales de una lista blanca, pero sus 6 pruebas seguían siendo las de antes — ninguna ejercía una condicional válida, un ID desconocido, ni una dependencia hacia una condicional ausente | Agregados 3 casos nuevos: condicional válida (`3B-7b`) pasa limpio; ID inventado (`3B-99`) falla aunque esté bien formado; dependencia hacia una condicional ausente falla con la misma regla que una dependencia base ausente — 9 casos en total | 3A-0b |
| 4 | El criterio de cierre seguía diciendo "las 40 filas del tracker" en varios puntos (3E-2, 3E-3, sección 7), sin contemplar que hasta 3 filas condicionales pueden existir además de las 40 base — un cierre podría declararse completo sin haber verificado una condicional activa | Reescrito uniformemente a "las 40 filas base del tracker más cualquier fila condicional presente" en los 4 puntos donde aparecía | 3E-2, 3E-3, sección 7 |
| 5 | 3B-6 reemplazó el `ORDER BY nombre` de Postgres por `localeCompare('es')` en Node, pero su prueba de equivalencia comparaba el resultado contra sí mismo (el mismo orden nuevo), nunca contra el comportamiento real anterior — no detectaría una diferencia de collation entre Postgres y JS | Nueva prueba: fixture chico (~50 filas, bajo el cap de `max_rows`) consultado con el código **viejo** (`.order('nombre')`, una sola página, refleja el `ORDER BY` real de Postgres) como referencia, comparado contra la función **nueva** sobre el mismo fixture — si difieren, el comparador se ajusta hasta igualar antes de aceptar el bloque | 3B-6 |
| 6 | Quedaban 2 "si lo hay" sin resolver: 3B-11 sobre un test existente de `ordenes-historial`, 3D-11 sobre un test existente del caso `TipoYaAsignadoError` | Confirmado por `Glob`/grep que ninguno de los 2 existe hoy — 3B-11 no depende de ningún test previo; 3D-11 crea `proyectos-tipo-route.test.ts` (nuevo, no existía) en vez de "extender" algo inexistente | 3B-11, 3D-11 |
| 7 | El título de 3B-5 y 4 filas del inventario de la sección 3 seguían diciendo `.range()` pese a que la especificación real (desde la ronda 6) es keyset | Título de 3B-5 y las filas de `getProyectos`/`getTareasAgregadas`/`getProveedores`/`sync-down.ts::syncTableDown()` corregidas a "keyset" | 3B-5, sección 3 |

## Tabla de auditoría — ronda 7

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | El validador de 3A-0b exigía exactamente 40 filas, pero el propio plan permite hasta 3 filas condicionales (`3B-7b`, `3C-4b`, `3D-0b`) que romperían esa cuenta al activarse | Validador ajustado: 40 IDs base obligatorios + hasta 3 IDs condicionales de una lista blanca cerrada (`3B-7b`/`3C-4b`/`3D-0b`) permitidos — cualquier otro ID falla; el criterio de cierre cuenta base + solo las condicionales que efectivamente aparecen | 3A-0b, sección 4, 7 |
| 2 | 3A-0 definía el mecanismo de 2 commits para cerrarse a sí mismo, pero 3A-6/3E-1/3E-2 (los otros 3 bloques doc-only) seguían declarando 1 solo commit — inconsistente con la regla uniforme recién escrita | Los 3 bloques ahora declaran explícitamente el mismo mecanismo de 2 commits (contenido + sincronización) | 3A-6, 3E-1, 3E-2 |
| 3 | 3A-1 decía "remapeo `TEST_*` → nombres de producción" sin la tabla exacta; la app en realidad lee `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`AUTH_SECRET`/nombres Google de runtime, no genéricos | Tabla exacta de 13 variables → origen (secreto de GitHub o literal), verificada contra el job `live` real de `e2e.yml`; `env-check` ampliado a anon key, service-role key (por fingerprint), spreadsheet, ambos folders de Drive, `AUTH_SECRET`, y verificación explícita de que el proyecto no es el de producción | 3A-1 |
| 4 | `prepare-portal-fixtures.mjs` pretendía llamar `signPortalSession()` de `lib/portal-auth.ts` (un `.ts`) desde un script `.mjs` corrido con `node` plano — confirmado que no hay `tsx`/`ts-node` en `package.json`, ni el alias `@/` es resoluble fuera de Next.js | Nuevo endpoint interno `POST /api/internal/loadtest-portal-session`, guardado por el mismo patrón fail-closed de `LOADTEST_MODE`/`LOADTEST_ENV_SECRET`, que sí corre dentro de Next.js y llama la función real sin duplicarla; más una prueba de paridad unitaria explícita `signPortalSession`↔`verifyPortalSession` | 3A-2 |
| 5 | El cleanup de reservas de folio en `bulk-cleanup.mjs` buscaba un prefijo `LOADTEST-${runId}-` en la columna `folio` (formato real `SH###`, nunca ese prefijo) y borraba por `id` — la tabla real tiene PK `token`, no `id`; el cleanup nunca habría borrado ninguna reserva | Reemplazado por `.in('folio', cotizacionIds)` directo (mismo patrón que `live-cleanup.ts`), reutilizando los IDs ya descubiertos — sin necesidad de descubrir nada por prefijo; reservas huérfanas no consumidas ya se autolimpian vía `expires_at` en la propia RPC de reserva; nueva prueba de que un folio puede volver a reservarse tras el cleanup | 3A-4 |
| 6 | `loadtest_runs` se creaba sin RLS (inconsistente con la convención del resto de tablas de datos); `LoadtestOverrideRejectedError` era un `Error` plano, así que el catch real de las 3 rutas (`buildErrorResponse`) le habría dado 500, no el 400 prometido | `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` + `REVOKE`/`GRANT` a `service_role` en `loadtest_runs`; `LoadtestOverrideRejectedError` ahora extiende `DomainError` con `status: 400`, así el catch genérico ya existente produce 400 sin cambios adicionales en las rutas | 3A-4 |
| 7 | La RPC de `pg_stat_statements` fijaba `search_path = public, pg_catalog`, pero Supabase instala esa extensión en el esquema `extensions`, no en `public` — la función habría fallado con "relation does not exist"; además `mean_exec_time_ms_after` era el promedio histórico, no el de la ventana de carga, y un `queryid` nuevo en "after" se etiquetaba "genuinamente nuevo" sin poder distinguirlo de una entrada evictada y reseteada | Verificación real de `table_schema` (no solo columnas) antes de escribir la RPC; `FROM extensions.pg_stat_statements` calificado explícitamente; `window_mean_ms = total_exec_time_delta_ms / calls_delta` (con guarda de división por cero) reemplaza la media histórica; filas nuevas en "after" se marcan `flag: 'new_or_reset'`, nunca "nuevas" sin calificar | 3A-5 |
| 8 | 3A-6/3E-1 corrían los 8 escenarios contra ambos entornos compartiendo la misma base/Drive de test sin ningún ciclo de reset entre uno y otro — el segundo entorno mediría sobre datos ya mutados por el primero, invalidando la comparación "lado a lado" | Ciclo obligatorio de 8 pasos por target (`runId` distinto, cleanup de huérfanos, seed idéntico, verificación de conteos iniciales idénticos, snapshot, k6, snapshot, cleanup), nunca solapado entre targets | 3A-6, 3E-1 |
| 9 | El cursor de `getProveedores()` (3B-6) interpolaba `nombre` (texto libre de proveedor) directo en un filtro `.or()` de PostgREST, que trata `,`/`(`/`)` como reservados — un nombre con esos caracteres rompería el filtro; el cursor nullable de `fecha_limite` en `getTareasAgregadas()` (3B-5) quedaba como "mismo patrón", sin las 2 ramas completas que un `NULLS LAST` real exige | 3B-6: cursor movido a PK inmutable (`id`, uuid, nunca texto arbitrario) con el orden final por `nombre` aplicado una sola vez en Node sobre el array ya completo; 3B-5: las 2 ramas del cursor nullable (cursor no-nulo incluye además todas las filas NULL; cursor ya en el bloque NULL solo avanza por `id`) escritas en código completo | 3B-5, 3B-6 |
| 10 | En 3C-2, el `select` de `syncTableDown` pedía solo `schema.columns`, pero `cuentas_cobrar`/`cuentas_pagar` no incluyen `created_at` en esa lista aunque sí lo tengan en Postgres — el cursor habría quedado `undefined` y el loop no habría avanzado para esas 2 tablas; en 3C-3, el catch real de `syncTableDown` (código actual, confirmado) convierte cualquier excepción en `{ok:false}`, así que `SheetsSyncLeaseLostError` nunca habría llegado a producir el 409 descrito | El `select` ahora siempre incluye la columna de cursor y el `pk` aunque no estén en `schema.columns` (el mapeo a filas de Sheets sigue filtrando solo por `schema.columns`); el catch de `syncTableDown` relanza `SheetsSyncLeaseLostError` explícitamente antes de convertir cualquier otro error en `{ok:false}` | 3C-2, 3C-3 |
| 11 (editorial) | El inventario de la sección 3 decía "10 con acción + 3 no aplica" sobre una tabla de 13 filas con 12 accionables + 1 fila agrupada de "no aplica" | Corregido a "12 con acción + 1 fila agrupada de no aplica" | Sección 3 |
| 12 (editorial) | Quedaban expresiones abreviadas ("mismo patrón", "si los hay", "o su envoltura equivalente") en puntos que el propio plan exige autosuficientes | 3D-1 cierra el tipo exacto de `pendingMutationsRef`/`trackMutation` contra el código real; 3B-6 confirma por grep que ninguno de sus 5 consumidores tiene test dedicado hoy (verificación manual explícita, no "si los hay") | 3D-1, 3B-6 |
| 13 (editorial) | No quedaba claro cómo un characterization test que descubre una pérdida real de dato puede seguir cerrando 3D-0 "en verde" sin contradecir que es un bloque test-only | Aclarado: si la predicción falla, el assert de ESE caso se reescribe para afirmar el valor realmente observado (el bug, documentado como tal) — el archivo cierra en verde documentando la realidad, pero el bloque queda `Bloqueado` en el tracker hasta que `3D-0b` (con un test que si debe ir de rojo a verde) resuelva el bug real | 3D-0 |
| 14 (editorial) | `CHUNK_SIZE=500` en `bulk-cleanup.mjs` para filtros `.in()` con UUIDs podía superar límites prácticos de URL de PostgREST (~18-20 KB codificados) | Bajado a `CHUNK_SIZE=150` (margen real), con una prueba empírica obligatoria contra `serenata-erp-test` antes de aceptar el bloque | 3A-4 |

## Tabla de auditoría — ronda 5

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | El tracker no distinguía bloques con PR de bloques doc-only | Campo "PR" pasa a `N/A` en filas doc-only, y se agrega un campo propio "Commit SHA" para esas filas; el validador de 3A-0b ajustado para exigir uno u otro según el tipo de bloque, nunca ambos vacíos | 3A-0, 3A-0b |
| 2 | Sin definir el "segundo commit" que registra el SHA del propio 3A-0, ni el orden exacto de sincronizar `En curso` a `main` + rebasar la rama | Definidos ambos procedimientos exactos, paso a paso | 3A-0 |
| 3 | Parámetros de k6 sin cerrar (`startRate`/`timeUnit`/`stages`/`preAllocatedVUs`/`maxVUs`/think time/duración) | Tabla con los valores exactos por escenario | 3A-5 |
| 4 | Telemetría de Supabase sin mecanismo concreto de captura de deltas por `queryid` | RPC segura `pg_stat_statements_snapshot()` + cálculo de deltas en Node, con migración/archivos/tests exactos | 3A-5 |
| 5 | Sin definir cómo las rutas reales de upload usan la carpeta Drive del `runId` de la carga | Override autenticado por header, activo solo bajo `LOADTEST_MODE`, con archivos y tests exactos | 3A-4 |
| 6 | El cleanup reutilizaba `live-cleanup.ts` sin paginación para volúmenes de miles de filas | Módulo propio de cleanup masivo (`bulk-cleanup.ts`), descubre IDs paginando y borra en chunks, nunca reutiliza las funciones de `live-cleanup.ts` para los volúmenes de 3A-3 | 3A-4 |
| 7 | Circuit breaker de 3B-5/3B-6 debía lanzar error, no devolver array parcial (ya corregido en la ronda 4, confirmado sin cambios adicionales) | Confirmado: ambos bloques ya lanzan `Error` explícito al agotar el circuit breaker | 3B-5, 3B-6 |
| 8 | 3B-9 sin orden explícito; 3B-12 con forma de salida ambigua (`jsonb_object_agg` colapsaba duplicados); 3B-11 con "confirmar al implementar" | Los 3 cerrados con SQL/shape exactos, verificados contra el código real | 3B-9, 3B-11, 3B-12 |
| 9 | 3C-3 no incluía `sync-down.ts` explícitamente; heartbeat solo entre tablas, no durante la lectura de una tabla grande; sin validar duración del cron contra el volumen objetivo | Heartbeat cableado dentro de `syncTableDown`/`syncAllDown` (por página, no solo por tabla); 3C-4 agrega medición empírica de duración contra `maxDuration` | 3C-3, 3C-4 |
| 10 | Faltaban los 17 tests que mockean `integrations/sheets/trigger` en el alcance de 3C-1 | Listados exactos, con la instrucción de quitarles el mock | 3C-1 |
| 11 | Sin `permissions: contents: write`, checkout apropiado ni guard de rama para el push a `loadtest-target` | Agregados explícitos al workflow de 3A-1 | 3A-1 |
| 12 | El resultado esperado del characterization test de reconciliación/transición (3D-0) quedaba abierto | Cerrado: análisis del código real (`reconciliarConServidor`, guards de dirty-ref, `escrituraLocalPosterior`) documentado como predicción verificable; si el test contradice la predicción y revela pérdida/carrera real, se convierte en hallazgo nuevo y se corrige antes de cerrar 3D-6, nunca se preserva un bug encontrado | 3D-0, 3D-6 |
| 13 | Bloques todavía abreviados ("mismo/igual patrón") | Expandidos a especificación completa | 3B-3 (ya en ronda 5 previa), 3D-3, 3D-4, 3D-11 |

## Tabla de auditoría — ronda 6

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | 3C-4 tenía los puntos 1-6 duplicados textualmente (artefacto de una edición previa) y dejaba la remediación del gate de duración como "diseño exacto a definir", violando la regla de "no placeholders" | Deduplicado a una sola especificación de 15 puntos; la remediación queda decidida ahora en 2 pasos fijos y completos (subir `maxDuration` al máximo del plan de Vercel; si no alcanza, chunking determinista vía `next_table_index` + cron horario + `CHUNK_TABLES=3`), sin ninguna decisión de diseño pendiente al momento de ejecutarla | 3C-4 |
| 2 | 3D-0 se declaraba test-only (un único archivo, sin tocar `page.tsx`) pero ordenaba corregir código dentro del mismo bloque si el characterization test de interleaving revelaba una carrera real — contradicción de alcance | 3D-0 nunca corrige código: si el test revela una carrera real, 3D-0 queda `Bloqueado` (no `Cerrado`), se agrega el hallazgo F26 (condicional) a la matriz, y se abre un bloque correctivo nuevo `3D-0b` con archivos/pruebas/rollback propios, ejecutado antes de que 3D-6 proceda | 3D-0, 3D-0b (condicional), 3D-6 |
| 3 | La explicación de 3A-0 sobre el "segundo commit" era contradictoria: decía que los demás bloques doc-only (3A-6, 3E-1, 3E-2) podían registrar su propio SHA en el mismo commit que los cierra, lo cual es imposible (un commit no conoce su propio hash mientras se escribe) | Regla única y uniforme para los 4 bloques doc-only, sin caso especial: todo cierre doc-only son 2 commits siempre — commit de contenido (cierra la fila con "Commit SHA" pendiente) + commit de sincronización inmediato siguiente (llena el SHA real ya existente) | 3A-0 |
| 4 | Error de conteo en la matriz de hallazgos: "25 filas con acción + F21 (no aplica) = 26" trataba a F21 como una fila adicional a las 25, cuando F21 ya es una de los 25 IDs (F1-F25) | Corregido: F1-F25 = 25 hallazgos base (24 con bloque + F21 no aplica), más hasta 3 condicionales (F14b, F15b, F26) = 25 a 28 según cuáles se activen, nunca "26 fijos" | Sección 4, 3E-2, 3E-3, sección 7 |
| 5 | El total de horas de EF-3B estaba mal sumado (120-140h en vez de 122-142h), y el total general no reflejaba los cambios de horas de la ronda 5 en 3A-4/3A-5/3C-2/3C-3 | Tabla de estimación recalculada desde cero contra las horas actuales de los 40 bloques (ver sección 8) | Sección 8 |
| 6 | La sección "Contexto" decía "cuarta versión" pese a que el documento ya iba en una ronda de corrección posterior | Corregido a reflejar la versión real del documento (v6) | Sección 1 |

## Tabla de auditoría — ronda 4

| # | Observación | Corrección | Bloque(s) |
|---|---|---|---|
| 1 | Conteo mal hecho: 3A-0..3A-6 son 7, no 6 | Recalculado desde cero (ver §2) | Estimación |
| 2 | 3A-0 declarado doc-only pero incluye un workflow de CI y un script `.mjs` — no son `.md`, requieren rama+PR | Dividido: **3A-0** (5 archivos `.md`, doc-only) y **3A-0b** (validador+workflow, rama+PR) | 3A-0, 3A-0b |
| 3 | "En curso" solo en la rama deja `main` desactualizado | Decidido explícitamente: `main` **sí** se sincroniza de inmediato — el primer commit de la rama de un bloque incluye también un commit-parche a `main` (doc-only, separado) marcando ese bloque `En curso` con el nombre de la rama; evita que alguien mirando `main` vea todo como `Pendiente` mientras hay trabajo real en curso | 3A-0 |
| 4 | Documento dependía de "ronda anterior" en varios bloques | Cada bloque de este documento trae su spec completa, sin referencias | Todo el documento |
| 5 | Inventario de colecciones incompleto: faltaban `getAllProveedorDocumentos()`, `getOrdenesPago()`, `getTareasAgregadas()` | Auditadas y clasificadas: la primera → agregado SQL; la segunda → paginación real; la tercera → mismo tratamiento que Proyectos (lectura interna completa vía `.range()`, la consume el mismo tablero) | 3B-12, 3B-11, 3B-5 |
| 6 | `.limit(5000)` en el cliente no vence `db.max_rows` de PostgREST (confirmado `max_rows=1000` en `supabase/config.toml:18`) | Proyectos/Proveedores/tareas agregadas ya no usan "límite alto + alerta" (no funciona) — leen **todas las páginas reales** con `.range()` en loop hasta agotar, con desempate estable, antes de responder; se verifica empíricamente el `max_rows` real de `serenata-erp-test` en vez de asumirlo | 3B-5, 3B-6 |
| 7 | Carga de volumen podía interferir con `live`/CI normal (comparten `serenata-erp-test`/Drive) | `load-test.yml` comparte grupo de `concurrency` de GitHub Actions con `e2e.yml` (nunca corren a la vez); `loadtest-target` se fija al SHA exacto del baseline con un push explícito antes de cada corrida | 3A-1, 3A-6, 3E-1 |
| 8 | Un workflow de GitHub Actions no puede invocar "MCP Supabase" — eso es una herramienta de la sesión de Claude, no de CI | Reemplazado por snapshots SQL directos (`pg_stat_statements`) antes/después de la ventana de carga vía el propio cliente `service_role` del job, y captura best-effort de `vercel logs` vía CLI con `VERCEL_TOKEN` — declarado explícitamente como best-effort, con fallback a captura manual documentada si el plan de Vercel no expone suficiente detalle | 3A-5 |
| 9 | Cleanup de Drive asumía una columna `drive_file_id` que no existe en `documentos_cuentas_cobrar`/`documentos_cuentas_pagar` (tienen `archivo_url`); además un archivo subido con INSERT fallido queda huérfano | Rediseño completo: **carpeta de Drive dedicada por `runId`**, creada al inicio de cada corrida dentro de `DRIVE_TEST_FOLDER_ID` — cleanup borra la carpeta completa (cascada), sin depender de ninguna columna de Postgres ni de que el INSERT haya tenido éxito | 3A-4 |
| 10 | `3B-1` no redondeaba el saldo a 2 decimales como `calcularSaldoPendiente` | SQL corregido con `ROUND(...,2)` | 3B-1 |
| 11 | Campos de búsqueda de Cotizaciones "se confirmarán al implementar" | Cerrados: `id`, `cliente`, `proyecto` (columna propia) + `descripcion`/`responsable_nombre` de `items_cotizacion` (vía `EXISTS`) — leído directo de `app/cotizaciones/page.tsx:66-79` | 3B-4 |
| 12 | Faltaban los conteos por estado (pestañas) independientes de la búsqueda | La RPC de Cotizaciones devuelve `counts_by_estado` sobre la tabla completa, la búsqueda solo afecta `rows`/`total_rows` | 3B-4 |
| 13 | `3B-8` seguía con nombres de columna "provisionales" | Cerrados con nombres reales, sin placeholders | 3B-8 |
| 14 | Faltaba escapar `%`/`_` en búsquedas `ILIKE`, y debounce/cancelación de respuestas atrasadas | Todas las RPCs de búsqueda escapan `%`/`_`/`\` con `ESCAPE '\'`; todos los hooks de búsqueda usan debounce de 300ms + `AbortController` + número de secuencia para descartar respuestas atrasadas | 3B-2, 3B-3, 3B-4, 3B-11 |
| 15 | Dashboard debía redondear cada saldo individual, no solo la suma final | SQL usa `ROUND(GREATEST(...),2)` por fila antes de `SUM`, y `ROUND(SUM(...),2)` otra vez encima — paridad exacta con el doble redondeo real de `dashboard.ts` | 3B-10 |
| 16 | Lock de Sheets: lease de 600s sin renovación (una sync de 9 tablas puede tardar más); `acquire` podía no devolver `false` con 0 filas afectadas; sin `renew`; endpoint de estado sin auth; error técnico crudo expuesto; `gen_random_uuid()` no puede pasarse como si fuera una llamada de TypeScript | Rediseño completo: `acquire`/`release`/`renew` en `plpgsql` con `GET DIAGNOSTICS` (garantiza `boolean` real); `renew_sheets_sync_lease` llamado tras cada tabla sincronizada; `GET /status` exige la misma auth que `POST /sync-down`; `error_message` guarda un resumen seguro armado por la ruta, nunca el error crudo (eso va solo al log); `run_id` se genera en Node con `crypto.randomUUID()` y se pasa como parámetro | 3C-3 |
| 17 | `3B-7`: si el gate de latencia serverless fallaba, el plan permitía "conservar el caché y documentar" — contradice que esta versión no difiere deuda | Si falla, el bloque queda `Bloqueado` (no `Cerrado`) y se agrega una fila nueva a la matriz (`F14b`) que debe resolverse o diferirse **con aprobación explícita** antes de cerrar EF-3 — nunca aceptado en silencio | 3B-7 |
| 18 | `3D-6`: interacción `flushPendingSaves`/`reconciliacionEnCursoRef` "se verificaría al implementar" | Auditado ahora: `reconciliacionEnCursoRef` (líneas 784-901 de `page.tsx`) es un guard interno de la propia reconciliación — **no lo lee ni lo espera** `flushPendingSaves` ni ninguna acción de negocio (`aprobar`/`generarPDF`/`generarCotizacion`/`cancelarCotizacion`), confirmado por grep completo. No hay sincronización explícita entre ambos hoy. Se agrega un characterization test específico en 3D-0 que documenta ese comportamiento real (una reconciliación en vuelo y un flush de transición pueden interleavearse sin guard mutuo) antes de extraer nada | 3D-0, 3D-6 |
...
| 3B-5 | En curso | ninguna | `claude/hopeful-allen-jql9xp` | [#39](https://github.com/EduardoTerwogt/serenata-erp/pull/39) | — | — | | Esperar CI en verde, luego mergear |
