...
const COLS_DOC = 'id, tipo, archivo_url, archivo_nombre, fecha_carga, estado_validacion, detalle_validacion'
...
export async function cargarDetalleCobro(id: string): Promise<DetalleCobro | null> {
  ...
  const [proyecto, docs, pagos] = await Promise.all([
    ...
    supabaseAdmin.from('pagos_comprobantes').select('id, monto, tipo_pago, fecha_pago, comprobante_url, notas, created_at').eq('cuentas_cobrar_id', id),
  ])
  if (docs.error) throw docs.error
  if (pagos.error) throw pagos.error

  return armarDetalleCobro({ cuenta, proyecto, documentos: (docs.data ?? []) as DocumentoFila[], pagos: pagos.data ?? [] }, hoyCdmx())
}
...
export async function cargarDetallePago(objetivo: 'grupo' | 'cuenta', id: string): Promise<DetallePago | null> {
  ...
  const [proyecto, docs, pagos, proveedor, orden] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .select('id, fecha_pago, tipo_pago, monto_transferido, comprobante_url, notas, estimado, created_at')
