...
export async function generarPDFCotizacion(data: PDFData): Promise<void> {
  ...
  try { isoLogoPng = await loadImageAsDataUrl(ISO_LOGO) } catch {}
  try { serenataLogoPng = await loadImageAsDataUrl(SERENATA_LOGO) } catch {}
  ...
  if (isoLogoPng) {
    ...
    try { doc.addImage(isoLogoPng, pageW - margin - isoW, 8, isoW, isoH) } catch {}
  }
  ...
  const totalsRows: TotalsRow[] = [
    ...
    ...(descuento > 0 ? [{ label: 'Descuento', value: `-${fmtPDF(descuento)}`, labelColor: GRAY, valueColor: WHITE, bold: false, fontSize: 8.5 }] : []),
    ...(data.iva_activo ? [{ label: 'IVA (16%)', value: fmtPDF(data.iva), labelColor: GRAY, valueColor: WHITE, bold: false, fontSize: 8.5 }] : []),
...
export async function generarHojaDeLlamado(data: HojaDeLlamadoData): Promise<void> {
  ...
  try { isoLogoPngHL = await loadImageAsDataUrl(ISO_LOGO) } catch {}
  try { serenataLogoPngHL = await loadImageAsDataUrl(SERENATA_LOGO) } catch {}
