-- Sync repo migrations with the SQL functions already corrected in production/Supabase.
-- This migration does not rewrite history: it versions the current working state.

CREATE OR REPLACE FUNCTION public.replace_cotizacion_items(p_cotizacion_id text, p_items jsonb)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  DELETE FROM public.items_cotizacion WHERE cotizacion_id = v_id;
...
GRANT EXECUTE ON FUNCTION public.save_cotizacion(jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.replace_cotizacion_items(text, jsonb) TO authenticated, service_role;
