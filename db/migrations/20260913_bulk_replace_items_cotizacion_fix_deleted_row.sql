-- Engineering Hardening EF-1 -- fix post-auditoría de PR #29 (2026-09-12).
-- Migración append-only sobre la RPC de
-- 20260912_bulk_replace_items_cotizacion.sql (ya aplicada a
-- serenata-erp-test y a producción): esa migración nunca se edita, esta la
-- reemplaza completa vía CREATE OR REPLACE.
--
-- Hallazgo: el chequeo de P1410 (conflicto de revision) solo comparaba
-- `v_current_revision` contra la esperada cuando la fila de
-- `p_reemplazar_ids` SÍ existía (`if found and ... is distinct from ...`).
-- Si esa fila fue borrada por una operación concurrente (p. ej. otro
-- colaborador borró la partida justo antes de que este bulk corriera), la
-- condición `found` era falsa y el chequeo se saltaba entero -- sin
-- conflicto, sin rechazo. Si esa misma id venía además en `p_items` (el
-- cliente, con el snapshot viejo, la seguía creyendo viva y planeaba
-- reutilizarla), el `INSERT ... ON CONFLICT (id) DO UPDATE` de más abajo
-- simplemente la insertaba de cero -- resucitando una fila que otra
-- operación había borrado a propósito, sin que nadie lo detectara.
--
-- Fix: una fila de `p_reemplazar_ids` que ya no existe es SIEMPRE conflicto
-- (P1410), sin importar qué `revision` traiga el payload -- no hay
-- `revision_actual` con la que comparar, y el hecho de que ya no exista
-- prueba que el snapshot del cliente quedó viejo. Mismo principio "todo o
-- nada" que ya aplicaba al conflicto por revision distinta.
