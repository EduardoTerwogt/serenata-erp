...
export function useQuotationCollaboration({
  ...
  const realtimeAvailable = Boolean(supabaseUrl && supabaseAnonKey)
  const supabaseClient = useMemo<SupabaseClient | null>(() => {
    if (!realtimeAvailable) return null
    return createClient(supabaseUrl!, supabaseAnonKey!)
  }, [realtimeAvailable, supabaseAnonKey, supabaseUrl])
  ...
  useEffect(() => {
    if (!enabled || !realtimeAvailable || !supabaseClient) return

    const random = Math.random().toString(36).slice(2, 8)
    presenceKeyRef.current = `${identity.userId}-${random}`

    const channel = supabaseClient.channel(`cotizacion:${cotizacionId}`, {
  ...
  }, [cotizacionId, enabled, identity.email, identity.name, identity.userId, logEvent, realtimeAvailable, supabaseClient])
