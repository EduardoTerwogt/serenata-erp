...
function NuevaCotizacionContent() {
  ...
  const onGenerarCotizacion = handleSubmit(async (data) => { if (isSubmitting.current) return; isSubmitting.current = true; setGuardando(true); setError(null); try { const cotizacion = await saveNewQuotation(data, { estado: 'EMITIDA', porcentaje_fee, iva_activo, descuento_tipo, descuento_valor, ...(esComplementaria ? { tipo: 'COMPLEMENTARIA' as const, es_complementaria_de: complementaria_de } : {}) }); const pdfResult = await generateQuotationPdf(cotizacion, data.items, { skipDownload: true }); if (pdfResult.savedToDrive) { sessionStorage.setItem('pdf_drive_result', JSON.stringify({ link: pdfResult.driveWebViewLink ?? null })) } router.push(`/cotizaciones/${cotizacion.id}`) } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error desconocido') } finally { isSubmitting.current = false; setGuardando(false) } })
